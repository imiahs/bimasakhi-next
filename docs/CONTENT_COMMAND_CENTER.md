...
> **Status:** ACTIVE - Foundation Release Aligned; Docs-Only Operational Baseline Synchronized; Reusable Runtime Closure Live-Validated and Repository-Closed; Continuous Live Convergence Operational; SHOS Live Authority Confirmed In Production  
> **Owner:** CTO (under CEO authority � Constitution Article 1)  
| **Last Updated:** May 26, 2026 (DOCS-ONLY FOUNDATION ALIGNMENT synchronized authoritative docs to the git-backed reusable-runtime closure on `af7d7c3`: live baseline is now `af7d7c3`, reusable runtime is operational and repository-closed, continuous live convergence is operational, catch-all resolver authority remains a separate high-blast baseline, rollback safety remains preserved through prior ready deployment `dpl_3Y8fu2wsELvYb3XN3aXYxJ58jfdb`, and permanent cadence is now small bounded change -> local validate -> deploy -> live validate -> rollback validate -> commit close)
| **SHOS HARDENING IMPLEMENTATION STATUS (May 21, 2026):** LIVE DEPLOYMENT COMPLETE, LIVE AUTHORITY VALIDATED. The first rollback-safe authoritative SHOS hardening layer is now active in production. Operational summary, forensic summary, escalation authority, replay authority, dead-letter authority, and operator-visible lists are explicitly separated. No runtime authority widening, no topology redesign, and no unrestricted AI activation were introduced.
| **SHOS RECONCILIATION STATUS (May 21, 2026):** RESOLVED IN DEPLOYED SHOS SURFACE. Former live `OBSERVABILITY_DRIFT_PRESENT` behavior is no longer governing production `overall_health`. Stale escalation residue remains visible as `STALE` and `FORENSIC` context rather than live degraded inheritance. Current deployed durability classification for this lane: `PARTIALLY_DURABLE` only because browser-level operator DOM proof remains narrower than API proof; runtime authority classification is otherwise live-authoritative.
| **FULL FORENSIC PLATFORM AUDIT STATUS (May 23, 2026):** COMPLETE AS READ-ONLY FORENSIC RECONSTRUCTION. No implementation, deployment, runtime mutation, App Router widening, catch-all widening, or admin authority expansion occurred. Current production truth: Vercel production build logs show `bimasakhi.com` built from branch `main`, commit `0d0a9cc`, on May 21, 2026 with Next.js 15.5.12 and static generation `126/126`; public `/api/status` returned `status=ok`, `overall_health=HEALTHY`, `environment=production`, `db=ok`, `qstash_configured=true`, `redis_configured=true`, `system_mode=normal`, `ai_enabled=false`, `followup_enabled=true`, `crm_auto_routing=true`, and `queue_paused=false`; sampled production error logs returned no entries. Authority truth: Gemini is the only runtime-authoritative AI provider; OpenAI adapter is inactive/dead; Claude/DeepSeek are not implemented; workflow model/provider controls are cosmetic until runtime consumes them; prompt authority is fragmented; admin settings/navigation/pages/blog/CCC/SHOS are real only within their consumed lanes; SEO/media/resources/assets remain partial; explicit App Router routes remain highest-authoritative; generated catch-all remains bounded; static public images remain file-owned; `/hero-woman-professional.jpg` and `/noise.png` are live 404 asset gaps. Authoritative artifacts: `docs/audits/FULL-FORENSIC-PLATFORM-AUDIT.md`, `docs/audits/AI-AUTHORITY-AUDIT.md`, `docs/audits/ADMIN-AUTHORITY-AUDIT.md`, `docs/audits/UI-FRAGMENTATION-AUDIT.md`, `docs/audits/DEPLOYMENT-TRUTH-AUDIT.md`, `docs/tests/LIVE-RUNTIME-VALIDATION.md`, `docs/tests/ADMIN-CONSUMPTION-VALIDATION.md`, `docs/tests/STATIC-DYNAMIC-CLASSIFICATION.md`, `docs/tests/PROVIDER-EXECUTION-VALIDATION.md`.
| **PAGE AUTHORITY RECOVERY STATUS (May 21, 2026):** DISCOVERY COMPLETE, RECOVERY PLANNING ONLY. Existing page/content authority is fragmented, not absent. Confirmed lanes: static file routes, helper-wrapped static routes, custom block pages, draft-to-`page_index` publishing, blog/resources lanes, and hidden/orphaned registry-homepage tooling. Static runtime pages remain outside unified admin authority, SEO continuity is partial, and resolver/prompt-engine deploy blockers remain explicit. No editor rebuild, no runtime authority widening, no mass migration, and no deployment topology redesign were authorized. Authoritative artifacts: `docs/audits/PAGE-AUTHORITY-RECOVERY.md` and `docs/tests/PAGE-CRUD-CAPABILITY-MAP.md`.
| **PAGE AUTHORITY UNIFICATION STATUS (May 21, 2026):** FIRST CANONICAL LAYER IMPLEMENTED LOCALLY, VALIDATED. `seo_overrides.route_path` is now the shared metadata lookup key; dynamic runtime metadata for catch-all generated pages, custom pages, and blog posts now consumes canonical URL, robots, and SEO overrides; tracked authority surfaces no longer import local-only `resolveRoute.js` or `promptEngine.js`; and `/api/admin/seo` plus `/admin/seo` now expose a canonical read-only authority registry that only permits edits where runtime override consumption is actually wired. Boundaries preserved: explicit file routes still win, custom pages remain canonically bounded to `/pages/[slug]`, no new universal CRUD surface was introduced, and no runtime topology redesign was authorized. Remaining fragility: inline static metadata routes remain classified but read-only. A later local production build completed successfully during static authority integration validation. Authoritative artifacts: `docs/audits/PAGE-AUTHORITY-UNIFICATION.md` and `docs/tests/PAGE-RUNTIME-VALIDATION.md`.
| **STATIC AUTHORITY INTEGRATION STATUS (May 21, 2026):** FIRST STATIC-RUNTIME AUTHORITY INTEGRATION LAYER IMPLEMENTED LOCALLY, VALIDATED. All 76 explicit non-dynamic `app/**/page.js` routes are now inventoried inside the canonical registry through tracked `app/api/admin/seo/route.js` logic. Each static route is now classified with static/hybrid/hidden ownership labels, boundary and durability classifications, visibility scope, and runtime owner file evidence. Helper-wrapped public static routes remain route-path override capable; inline static routes are registry-visible but explicitly read-only, preventing fake metadata authority. `page_index` shadow detection now uses the full explicit static route inventory rather than a partial public-route list. Boundaries preserved: explicit static file routes remain production-authoritative, registry visibility does not replace runtime ownership, `/admin/pages` was not widened into universal static CRUD, and hidden/internal admin routes remain classified without being promoted into public SEO ownership surfaces. Remaining fragility: inline static metadata routes are still not helper-wired and override-only orphan paths remain authority-fragile. Local `npm run build` completed successfully after this integration. Authoritative artifacts: `docs/audits/STATIC-AUTHORITY-INTEGRATION.md` and `docs/tests/STATIC-RUNTIME-VALIDATION.md`.
| **STATIC AUTHORITY BASELINE CHECKPOINT (May 21, 2026):** CONFIRMED ON `main` AS `288cf959aafebc99b154d637ca2d7806082cde03` (`feat: static authority integration and successful production build pass`). This is the rollback-safe checkpoint required before bounded editable-authority enablement work.
| **EDITABLE AUTHORITY ENABLEMENT STATUS (May 21, 2026):** FIRST BOUNDED EDITABLE-AUTHORITY LAYER IMPLEMENTED LOCALLY, VALIDATED. `app/api/admin/seo/route.js` now emits bounded editability classifications, editability durability, layout protection, and deterministic editor targets; `/admin/seo` now exposes a canonical editable-authority registry with summary counts, bounded filters, and deep-link opening through `?path=`; `/admin/pages` and `/admin/blog` now expose canonical URL and robots controls through existing `?edit=` metadata lanes. Boundaries preserved: explicit route files and existing runtime renderers remain production-authoritative, editable visibility does not create layout ownership, `/admin/pages` was not widened into universal static CRUD, and no new source-editing or drag-drop runtime editor was introduced. Remaining fragility: inline static routes remain largely read-only, override-only orphan paths remain fragile, and helper-backed SEO-only surfaces remain only partially editable. Final validation: clean local `npm run build` completed successfully after removing generated `.next` output. Authoritative artifacts: `docs/audits/EDITABLE-AUTHORITY-ENABLEMENT.md` and `docs/tests/EDITABLE-RUNTIME-VALIDATION.md`.
| **STRUCTURED CONTENT AUTHORITY STATUS (May 23, 2026):** DETERMINISTIC STRUCTURED-CONTENT AUTHORITY RECONSTRUCTION COMPLETE IN BOUNDED LOCAL MODE, VALIDATED. The structured authority baseline now explicitly covers: complete surface discovery, deterministic trace model, explicit classification with no unclassified surfaces, canonical precedence ordering, ownership reconstruction, snapshot/version readiness, structured consumption continuity, and future block-foundation readiness. Boundaries preserved: runtime continuity remains authoritative, metadata ownership remains distinct from layout ownership, App Router precedence remains unchanged, catch-all precedence remains unchanged, registry visibility remains non-runtime-authoritative, and no visual builder, block-renderer rollout, or runtime authority widening was introduced. Residual fragility remains explicit for generated rows without linked draft ownership, inline read-only static metadata lanes, and override-only orphan registry rows. Authoritative artifacts: `docs/audits/STRUCTURED-CONTENT-AUTHORITY.md` and `docs/tests/STRUCTURED-RUNTIME-VALIDATION.md`.
| **OWNERSHIP LINKAGE + SNAPSHOT HARDENING STATUS (May 23, 2026):** DETERMINISTIC OWNERSHIP-CONTINUITY AND SNAPSHOT-SURVIVABILITY RECONSTRUCTION COMPLETE IN BOUNDED LOCAL MODE, VALIDATED. The ownership hardening baseline now explicitly covers: ownership continuity discovery, ownership trace model, ownership classification model, snapshot survivability classes, ownership lineage model, generated-runtime durability boundaries, ownership precedence model, deterministic inheritance continuity, trust durability classes, snapshot recovery durability, phased bounded implementation sequencing, and ownership survivability validation. Boundaries preserved: runtime continuity remains highest-authoritative, App Router precedence remains unchanged, catch-all precedence remains unchanged, generated-runtime surfaces do not implicitly become authoritative, registry visibility remains non-runtime-authoritative, and no runtime renderer mutation or authority widening was introduced. Residual fragility remains explicit for generated runtime rows without deterministic linked ownership, partially fragmented helper inheritance lanes, partially durable snapshot chains outside versioned owners, and override-only orphan ownership rows. Authoritative artifacts: `docs/audits/OWNERSHIP-LINKAGE-FOUNDATION.md` and `docs/tests/OWNERSHIP-RUNTIME-VALIDATION.md`.
| **LINEAGE RECOVERY + SNAPSHOT DURABILITY HARDENING STATUS (May 23, 2026):** DETERMINISTIC LINEAGE-CONTINUITY AND ROLLBACK-SAFE SNAPSHOT-RECOVERY RECONSTRUCTION COMPLETE IN BOUNDED LOCAL MODE, VALIDATED. The lineage hardening baseline now explicitly covers: lineage continuity discovery, lineage trace model, lineage classification model, snapshot chain durability classes, lineage reconstruction risk controls, snapshot recovery validation, lineage precedence hierarchy, deterministic lineage inheritance model, recovery durability classes, generated-runtime recovery durability boundaries, phased bounded implementation sequencing, and lineage survivability validation. Boundaries preserved: runtime continuity remains highest-authoritative, App Router precedence remains unchanged, catch-all precedence remains unchanged, generated-runtime lineage does not implicitly outrank explicit runtime precedence, registry visibility remains non-authoritative, and no runtime renderer mutation or authority widening was introduced. Residual fragility remains explicit for generated-runtime rows without deterministic linked lineage owners, partially durable snapshot chains outside deterministic version lanes, helper inheritance fragmentation on mixed static lanes, and orphan override-only lineage rows. Authoritative artifacts: `docs/audits/LINEAGE-RECOVERY-HARDENING.md` and `docs/tests/LINEAGE-RUNTIME-VALIDATION.md`.
| **PERSISTENCE CONVERGENCE STATUS (May 24, 2026):** BOUNDED CANONICAL PERSISTENCE-CONVERGENCE FOUNDATION IMPLEMENTED LOCALLY, VALIDATED. The active reusable storage contract remains legacy-first `page_blocks.block_data`, but `lib/blocks/registry.js` now exposes compatibility-governed persistence adapters that can read legacy-only, legacy-plus-shadow, and canonical-envelope block payloads without changing runtime render authority. Save now writes legacy payloads plus bounded canonical shadow metadata (`__canonical_shadow`, `__persistence`) so future canonical storage evolution can remain adapter-driven and rollback-traceable. `page_versions` snapshot replay remains Rule 16 ordered and now replays shadowed blocks safely through the same registry adapter path. Boundaries preserved: no forced storage migration, no destructive rewrite, no homepage persistence mutation, no App Router precedence change, no catch-all precedence change, and no persistence-authoritative rendering. Final validation: archived-only disposable dataset passed create/save/preview/rollback/replay; local preview-token route returned `200`; clean post-change `npm run build` passed with compile, lint/type validation, page-data collection, static generation `126/126`, build traces, and final page optimization. Authoritative artifacts: `docs/audits/PERSISTENCE-CONVERGENCE-AUDIT.md`, `docs/audits/DUAL-READ-DUAL-WRITE-AUDIT.md`, `docs/audits/PREVIEW-HYDRATION-PERSISTENCE-AUDIT.md`, `docs/audits/PERSISTENCE-GOVERNANCE-AUDIT.md`, `docs/tests/PERSISTENCE-CONVERGENCE-VALIDATION.md`, `docs/tests/ROLLBACK-PERSISTENCE-VALIDATION.md`, `docs/tests/PREVIEW-PERSISTENCE-VALIDATION.md`, and `docs/tests/HYDRATION-PERSISTENCE-VALIDATION.md`.
| **CROSS-LAYER SURVIVABILITY STATUS (May 25, 2026):** BOUNDED CROSS-LAYER SURVIVABILITY HARDENING IMPLEMENTED LOCALLY, VALIDATED. `app/pages/[slug]/page.js` still keeps published reusable reads bounded under `unstable_cache(..., { revalidate: 60 })`, keeps preview reads bounded under `noStore()`, and now also exposes request-scope governance in the hidden runtime marker through `data-request-scope-governance="preview_request_scoped_memoized"` for preview and `data-request-scope-governance="not_requested"` for public replay. Production-mode validation on the current build confirmed: public replay stayed on cached version `A`, preview surfaced fresh `B`, preview surfaced rolled-back `A`, two concurrent preview reads after the persisted update both returned the same fresh state, invalid preview token rejection still returned `404` and logged under `system:runtime-economics`, unknown block skip still returned `200` for known content and logged under `system:runtime-economics`, current-build PSI-facing asset durability still passed, and the same build artifact preserved replay behavior after a cold `next start` restart. Boundaries preserved: homepage untouched, catch-all runtime untouched, `/pages/[slug]` remained on-demand (`ƒ`), `/[...slug]` remained dynamic (`ƒ`), no runtime-authority widening, no uncontrolled invalidation, no hidden replay orchestration, and no observability-coupled execution dependency. Residual limit: mixed multi-instance deployment replay and degraded-observability behavior remain explicitly unproven in this local slice. Authoritative artifacts: `docs/audits/CROSS-LAYER-SURVIVABILITY-AUDIT.md`, `docs/audits/REPLAY-ROLLBACK-SURVIVABILITY-AUDIT.md`, `docs/audits/CACHE-ISR-SURVIVABILITY-AUDIT.md`, `docs/audits/HYDRATION-COMPUTE-SURVIVABILITY-AUDIT.md`, `docs/audits/PREVIEW-ISOLATION-SURVIVABILITY-AUDIT.md`, `docs/audits/OBSERVABILITY-CONFIDENCE-SURVIVABILITY-AUDIT.md`, `docs/audits/COMPUTE-AMPLIFICATION-SURVIVABILITY-AUDIT.md`, `docs/audits/OPTIMIZATION-REVERSIBILITY-SURVIVABILITY-AUDIT.md`, `docs/audits/RUNTIME-ECONOMICS-SURVIVABILITY-AUDIT.md`, `docs/audits/DEPLOYMENT-REPLAY-SURVIVABILITY-AUDIT.md`, `docs/tests/CROSS-LAYER-SURVIVABILITY-VALIDATION.md`, `docs/tests/REPLAY-ROLLBACK-VALIDATION.md`, `docs/tests/CACHE-REPLAY-VALIDATION.md`, `docs/tests/ISR-REPLAY-VALIDATION.md`, `docs/tests/HYDRATION-REPLAY-VALIDATION.md`, `docs/tests/PREVIEW-ISOLATION-VALIDATION.md`, `docs/tests/OBSERVABILITY-CONFIDENCE-VALIDATION.md`, `docs/tests/COMPUTE-DURABILITY-VALIDATION.md`, `docs/tests/PSI-DURABILITY-VALIDATION.md`, and `docs/tests/DEPLOYMENT-REPLAY-VALIDATION.md`.
| **INVALIDATION SURVIVABILITY STATUS (May 25, 2026):** BOUNDED ROUTE-OWNED SAVE-TIME INVALIDATION HARDENING IMPLEMENTED LOCALLY, VALIDATED. The reusable public replay lane now uses per-slug tagged cache entries, and the route-owned admin mutation surfaces (`PATCH`, `PUT`, `DELETE`, create, and bulk status update) now trigger bounded invalidation through a shared helper after successful persistence. Production-mode local validation confirmed authenticated admin slug mutation invalidated both old and new replay targets immediately (`old 200->404`, `new 200`), preview remained `preview_no_store` with request-scoped governance preserved, and invalidation payloads remained inspectable in API responses. Boundaries preserved: no centralized invalidation orchestrator, no distributed invalidation topology, no catch-all precedence change, no preview-authority widening, and no runtime-authority widening outside the reusable lane. Residual limit: mixed-instance deployment-overlap invalidation timing remains partially unproven. Authoritative artifacts: `docs/audits/SAVE-TIME-INVALIDATION-SURVIVABILITY-AUDIT.md`, `docs/audits/REPLAY-WINDOW-INVALIDATION-AUDIT.md`, `docs/audits/ROLLBACK-VISIBLE-INVALIDATION-AUDIT.md`, `docs/audits/INVALIDATION-OBSERVABILITY-AUDIT.md`, `docs/audits/DEPLOYMENT-OVERLAP-INVALIDATION-AUDIT.md`, `docs/audits/INVALIDATION-REVERSIBILITY-AUDIT.md`, `docs/audits/INVALIDATION-CROSS-LAYER-AUDIT.md`, `docs/audits/INVALIDATION-REALISM-AUDIT.md`, `docs/tests/SAVE-TIME-INVALIDATION-VALIDATION.md`, `docs/tests/REPLAY-WINDOW-VALIDATION.md`, `docs/tests/ROLLBACK-VISIBLE-INVALIDATION-VALIDATION.md`, `docs/tests/INVALIDATION-OBSERVABILITY-VALIDATION.md`, `docs/tests/DEPLOYMENT-OVERLAP-VALIDATION.md`, `docs/tests/INVALIDATION-REPLAY-VALIDATION.md`, `docs/tests/INVALIDATION-REVERSIBILITY-VALIDATION.md`, and `docs/tests/INVALIDATION-CROSS-LAYER-VALIDATION.md`.
| **DOCS-ONLY FOUNDATION ALIGNMENT STATUS (May 26, 2026):** CCC, INDEX, and the foundation-release summary docs are now synchronized to the closed reusable-runtime wave without touching runtime, replay, invalidation, rollback, or catch-all code. This wave exists only to remove stale partial-closure wording, align rollback-anchor language, and document the permanent small-wave operational doctrine. Final classification: `ALIGNED`, docs blast-radius `DOCS_ONLY_SAFE`, closure state `READY_TO_CLOSE`.
| **REUSABLE RUNTIME CLOSURE STATUS (May 26, 2026):** The reusable `/pages` lane is now live on `https://bimasakhi.com` from git-backed production baseline `af7d7c3`, with public replay continuity, preview/public separation, route-owned invalidation, save-restore rollback, final slug restoration, and repository closure all proven. The dedicated target `/pages/live-reusable-activation-93648961` remains restored and healthy after the final validation pass. Final classification: `SAFE`, live deployment `LIVE_SAFE`, rollback `ROLLBACK_SAFE`, repository state `CLOSED`.
| **HISTORICAL RC / CONVERGENCE NOTE (May 26, 2026):** RC-marked and convergence-marked entries below remain continuity history only. They are not the active operating mode after the reusable-runtime closure on `af7d7c3`.
| **RELEASE CONVERGENCE STATUS (May 25, 2026):** CONTROLLED RELEASE-CONVERGENCE GOVERNANCE CYCLE COMPLETE WITH PARTIAL LIVE AUTHORIZATION. Live production control-plane realism was confirmed (`/api/status` and `/api/health` stable on `0d0a9cc`), overlap realism for status lane was confirmed across 20 uncached samples with no version-health drift, reusable replay and live invalidation continuity remained partially unproven because current live reusable route targets were not observed (`/pages` sitemap entries absent and sampled slug `404`), PSI evidence remained blocked by external rate limiting (`429` on mobile and desktop PageSpeed API calls), rollback remained structurally prepared but unexecuted in this cycle, and repository convergence was explicitly classified into authoritative versus transitional surfaces. Final release classification: `PARTIALLY_VALIDATED` with explicit future-readiness gates. Authoritative artifacts: `docs/audits/RELEASE-CONVERGENCE-AUDIT.md`, `docs/audits/LIVE-RUNTIME-REALISM-AUDIT.md`, `docs/audits/DEPLOYMENT-OVERLAP-REALISM-AUDIT.md`, `docs/audits/LIVE-PSI-RUNTIME-AUDIT.md`, `docs/audits/RELEASE-ROLLBACK-AUDIT.md`, `docs/audits/REPOSITORY-CONVERGENCE-AUDIT.md`, `docs/tests/LIVE-DEPLOYMENT-VALIDATION.md`, `docs/tests/LIVE-REPLAY-VALIDATION.md`, `docs/tests/LIVE-INVALIDATION-VALIDATION.md`, `docs/tests/LIVE-PSI-VALIDATION.md`, `docs/tests/RELEASE-ROLLBACK-VALIDATION.md`, and `docs/tests/DEPLOYMENT-OVERLAP-LIVE-VALIDATION.md`.
| **RC RELEASE-CANDIDATE CONVERGENCE STATUS (May 25, 2026):** RELEASE-CANDIDATE CONVERGENCE GOVERNANCE CYCLE COMPLETE WITH PARTIAL LIVE AUTHORIZATION. Repository authority was explicitly classified into governance-authoritative, runtime-authoritative, rendering-authoritative, workflow-authoritative, systemic-shared-authoritative, and operational-authoritative groups; blast-radius, rollback complexity, replay risk, deployment risk, survivability impact, operational visibility, and authority escalation risk were mapped for each group. Live deployment realism remained stable on `0d0a9cc`; control-plane overlap was `OVERLAP_SAFE`; reusable lane overlap and invalidation timing remained `OVERLAP_UNPROVEN`; PSI remained `PSI_FRAGMENTED`; rollback execution realism remained `PARTIALLY_VALIDATED` because no rollback run occurred in this cycle. Final RC readiness classification: `PARTIALLY_VALIDATED` with explicit SAFE TO DEPLOY, SAFE AFTER LIVE VALIDATION, and NOT SAFE YET boundaries documented. Authoritative artifacts: `docs/audits/RC-READINESS-AUDIT.md`, `docs/audits/REPOSITORY-AUTHORITY-AUDIT.md`, `docs/audits/LIVE-DEPLOYMENT-REALISM-AUDIT.md`, `docs/audits/LIVE-PSI-ECONOMICS-AUDIT.md`, `docs/audits/RELEASE-ROLLBACK-REALISM-AUDIT.md`, `docs/audits/OPERATIONAL-BLAST-RADIUS-AUDIT.md`, `docs/tests/RC-LIVE-VALIDATION.md`, `docs/tests/LIVE-REPLAY-CONVERGENCE-VALIDATION.md`, `docs/tests/LIVE-INVALIDATION-CONVERGENCE-VALIDATION.md`, `docs/tests/LIVE-PSI-CONVERGENCE-VALIDATION.md`, `docs/tests/RELEASE-ROLLBACK-CONVERGENCE-VALIDATION.md`, and `docs/tests/DEPLOYMENT-OVERLAP-CONVERGENCE-VALIDATION.md`.
| **RC2 LIVE RUNTIME CONVERGENCE STATUS (May 25, 2026):** RC2 CYCLE COMPLETE WITH PARTIAL LIVE AUTHORIZATION. Live control-plane overlap remained stable across 25 samples on `0d0a9cc`; live reusable runtime validation remained incomplete because sitemap and direct probes exposed no reusable `/pages` 200 targets in this cycle; reusable replay and invalidation continuity therefore remained unproven in live production; rollback execution realism remained partial because no live rollback run occurred in this cycle; PSI remained externally blocked (`429` mobile and desktop). Repository convergence cleanup classification was updated with active authoritative, transitional, archival, and removable-candidate boundaries while preserving continuity intelligence. Final RC2 classification: `PARTIALLY_VALIDATED` and `REUSABLE_RUNTIME_UNPROVEN`. Authoritative artifacts: `docs/audits/RC2-LIVE-REUSABLE-RUNTIME-AUDIT.md`, `docs/audits/RC2-DEPLOYMENT-OVERLAP-AUDIT.md`, `docs/audits/RC2-ROLLBACK-REALISM-AUDIT.md`, `docs/audits/RC2-LIVE-PSI-AUDIT.md`, `docs/audits/RC2-REPOSITORY-CONVERGENCE-AUDIT.md`, `docs/audits/RC2-OPERATIONAL-READINESS-AUDIT.md`, `docs/tests/RC2-LIVE-REUSABLE-VALIDATION.md`, `docs/tests/RC2-LIVE-REPLAY-VALIDATION.md`, `docs/tests/RC2-LIVE-INVALIDATION-VALIDATION.md`, `docs/tests/RC2-LIVE-ROLLBACK-VALIDATION.md`, `docs/tests/RC2-LIVE-PSI-VALIDATION.md`, and `docs/tests/RC2-DEPLOYMENT-OVERLAP-VALIDATION.md`.
| **RC3 OPERATIONAL PRODUCTION CONVERGENCE STATUS (May 25, 2026):** RC3 CYCLE COMPLETE WITH PARTIAL LIVE AUTHORIZATION. Production topology for reusable `/pages` remained fragmented: sitemap layers exposed zero reusable `/pages` URLs and sampled direct reusable candidates remained `404`, so reusable runtime-marker discoverability, replay observability, and invalidation visibility remained unproven in this cycle. Control-plane overlap remained stable across 30 samples on `0d0a9cc` and stayed `OVERLAP_SAFE` for that lane only. Rollback choreography realism remained partial because no live reusable rollback execution occurred in RC3. PSI remained externally blocked with `429` for homepage and reusable candidate requests. Governance-density saturation was quantified (`258` audits, `157` tests) and RC3 command surfaces were pinned as active authority with archive-first cleanup posture to avoid deleting continuity intelligence. Final RC3 classification: `PARTIALLY_VALIDATED`, production topology `TOPOLOGY_FRAGMENTED`, governance-density `PARTIALLY_CONVERGED`. Authoritative artifacts: `docs/audits/RC3-PRODUCTION-TOPOLOGY-AUDIT.md`, `docs/audits/RC3-REUSABLE-REPLAY-AUDIT.md`, `docs/audits/RC3-ROLLBACK-CHOREOGRAPHY-AUDIT.md`, `docs/audits/RC3-GOVERNANCE-DENSITY-AUDIT.md`, `docs/audits/RC3-LIVE-PSI-AUDIT.md`, `docs/audits/RC3-OPERATIONAL-READINESS-AUDIT.md`, `docs/tests/RC3-LIVE-TOPOLOGY-VALIDATION.md`, `docs/tests/RC3-LIVE-REPLAY-VALIDATION.md`, `docs/tests/RC3-LIVE-INVALIDATION-VALIDATION.md`, `docs/tests/RC3-LIVE-ROLLBACK-VALIDATION.md`, `docs/tests/RC3-LIVE-PSI-VALIDATION.md`, and `docs/tests/RC3-GOVERNANCE-CONVERGENCE-VALIDATION.md`.
| **LIVE REUSABLE ACTIVATION MODE STATUS (May 25, 2026):** FIRST REAL PRODUCTION REUSABLE ACTIVATION EXECUTED WITH PARTIAL LIVE AUTHORIZATION. A new published reusable target `/pages/live-reusable-activation-93648961` was created in production and validated through immediate activation, mutation, and rollback sequence: activation marker `A` was visible at `200`, mutation marker `B` was visible at `200`, and rollback restored marker `A` at `200`. This cycle therefore converts reusable activation, replay continuity, and rollback continuity from inferred to directly observed on a live reusable target. Residual boundaries remain explicit: hidden reusable governance attributes were not observed in activated target HTML response, sitemap reusable discoverability remains absent in current shards, and PSI scoring remained blocked by external PageSpeed quota (`429`). Final LIVE ACTIVATION classification: `PARTIALLY_VALIDATED`, activation and replay lanes `PROVEN`, governance-marker and PSI lanes `PARTIAL_OR_EXTERNAL_BLOCKED`. Authoritative artifacts: `docs/audits/LIVE-REUSABLE-ACTIVATION-AUDIT.md`, `docs/audits/LIVE-REPLAY-INVALIDATION-AUDIT.md`, `docs/audits/LIVE-PSI-PRODUCTION-AUDIT.md`, `docs/audits/CONTINUOUS-CONVERGENCE-AUDIT.md`, `docs/tests/LIVE-REUSABLE-ACTIVATION-VALIDATION.md`, `docs/tests/LIVE-REPLAY-INVALIDATION-VALIDATION.md`, `docs/tests/LIVE-PSI-VALIDATION.md`, and `docs/tests/CONTINUOUS-CONVERGENCE-VALIDATION.md`.
| **CONTINUOUS LIVE CONVERGENCE BASELINE STATUS (May 26, 2026):** CONTINUOUS LIVE CONVERGENCE IS NOW OPERATIONAL ON `af7d7c3`. `/api/status` remains healthy, the dedicated reusable target remains `200` with original marker continuity, replay and invalidation continuity are operational on the reusable lane, and rollback remains operationally safe after final git-backed validation. Permanent doctrine is now explicit and stable: small bounded change -> local validate -> deploy -> live validate -> rollback validate -> commit close. Final baseline classification: `ALIGNED`, rollback `ROLLBACK_SAFE`, runtime health `OPERATIONAL`, cadence `SMALL_WAVE_ENFORCED`. Authoritative artifacts: `docs/audits/CONTINUOUS-CONVERGENCE-AUDIT.md`, `docs/audits/LIVE-RUNTIME-HEALTH-AUDIT.md`, `docs/audits/OPERATIONAL-CLARITY-AUDIT.md`, `docs/tests/CONTINUOUS-CONVERGENCE-VALIDATION.md`, `docs/tests/LIVE-ROLLBACK-VALIDATION.md`, and `docs/tests/LIVE-RUNTIME-HEALTH-VALIDATION.md`.
| **RELEASE HYGIENE + COMMIT CONVERGENCE STATUS (May 26, 2026):** THE REUSABLE-RUNTIME CLOSURE RISK HAS BEEN REMOVED FROM THE ACTIVE RELEASE SURFACE. `af7d7c3` is committed, pushed, and live-validated; the reusable runtime/admin/shared-lib cluster is no longer pending deployment. Remaining active isolation work is limited to docs-only alignment and separate catch-all, SEO, and wider local review surfaces. Historic backlog inflation remains a readability concern, but it is no longer allowed to mix with reusable-runtime closure. Final classification: `ALIGNED`, docs-only closure `DOCS_ONLY_SAFE`, reusable runtime closure `CLOSED`, blast-radius state `ISOLATED_BY_COMMIT_GROUP`.
| **RELEASE BASELINE NORMALIZATION STATUS (May 26, 2026):** THE FOUNDATION BASELINE IS NOW SYNCHRONIZED TO LIVE GIT-BACKED TRUTH. Trusted live baseline is `af7d7c3`; reusable runtime closure is committed, pushed, and live-validated; catch-all resolver authority remains a separate `HIGH_BLAST_RADIUS` baseline; prior ready deployment `dpl_3Y8fu2wsELvYb3XN3aXYxJ58jfdb` remains the preserved deterministic rollback anchor; and governance backlog reduction remains update-in-place plus archive-first. Recommended stable baseline tag: `foundation-2026-05-26-af7d7c3-stable`. Final classification: `ALIGNED`, reusable runtime `CLOSED`, catch-all `SEPARATE_BASELINE`, rollback anchor `PRESERVED`.
| **FOUNDATION RELEASE CLOSURE STATUS (May 26, 2026):** FOUNDATION RELEASE DOCUMENTATION IS NOW CLOSED AROUND THE LIVE GIT-BACKED REUSABLE-RUNTIME WAVE. Live runtime baseline is `af7d7c3`; reusable runtime closure is deployed, replay-safe, invalidation-safe, rollback-safe, and repository-closed; catch-all route authority and resolver helpers remain explicit separate-baseline work and are not part of this closure. Recommended foundation checkpoint after this docs-only closure: `foundation-2026-05-26-af7d7c3-stable`. Final classification: `ALIGNED`, docs bundle `DOCS_ONLY_SAFE`, reusable runtime `CLOSED`, catch-all `SEPARATE_BASELINE`.
| **ACTIVE RUNTIME OPTIMIZATION STATUS (May 25, 2026):** BOUNDED ACTIVE RUNTIME OPTIMIZATION HARDENING IMPLEMENTED LOCALLY, VALIDATED. `app/pages/[slug]/page.js` still separates published reusable-page caching from preview no-store reads through `unstable_cache(..., { revalidate: 60 })` plus `noStore()`, while the preview path now also reduces repeated metadata plus page-body store or adapter execution through request-scoped memoization on the same route-owned store reader. The reusable route still emits a hidden server-rendered runtime-policy marker that exposes runtime authority, render mode, cache governance, preview isolation, normalization mode, and optimization reversibility without changing route ownership. `components/blocks/PageBlocks.jsx` and `lib/blocks/registry.js` still avoid duplicate normalization on the authoritative render path by reusing route-normalized payloads and normalized canonical projection input. Hero and media asset markup remains additive and rollback-safe under the existing raw `<img>` authority model. Boundaries preserved: homepage untouched, catch-all runtime untouched, `/pages/[slug]` remains on-demand (`ƒ`) rather than promoted into static route authority, no runtime-authority widening, no uncontrolled cache orchestration, no request-time compute telemetry flood, and no forced asset-source policy cutover. Final validation: archived preview memoization PASS on `next dev`, production-mode active replay plus rollback PASS on `next start`, cache replay PASS, preview isolation PASS, hydration optimization PASS, compute durability PASS, PSI durability PASS, and clean `npm run build` PASS with compile, lint or type validation, page-data collection, static generation `126/126`, build traces, and final page optimization. Existing `jose` Edge Runtime warnings remain pre-existing and unrelated to this slice. Authoritative artifacts: `docs/audits/ACTIVE-RUNTIME-OPTIMIZATION-AUDIT.md`, `docs/audits/CACHE-ISR-OPTIMIZATION-AUDIT.md`, `docs/audits/HYDRATION-OPTIMIZATION-AUDIT.md`, `docs/audits/PSI-ASSET-OPTIMIZATION-AUDIT.md`, `docs/audits/COMPUTE-AMPLIFICATION-AUDIT.md`, `docs/audits/OPTIMIZATION-REVERSIBILITY-AUDIT.md`, `docs/tests/ACTIVE-RUNTIME-OPTIMIZATION-VALIDATION.md`, `docs/tests/CACHE-REPLAY-VALIDATION.md`, `docs/tests/PREVIEW-ISOLATION-VALIDATION.md`, `docs/tests/HYDRATION-OPTIMIZATION-VALIDATION.md`, `docs/tests/COMPUTE-DURABILITY-VALIDATION.md`, and `docs/tests/PSI-DURABILITY-VALIDATION.md`.
| **CANONICAL REUSABLE CONTRACT SURVIVABILITY STATUS (May 24, 2026):** BOUNDED CANONICAL CONTRACT EVOLUTION LAYER IMPLEMENTED LOCALLY, VALIDATED. The existing reusable runtime lane still persists block-specific `block_data`, but `lib/blocks/registry.js` now exports additive canonical projection helpers (`projectCanonicalBlockContract`, `projectCanonicalBlockRecord`, `projectCanonicalBlockRecords`) that deterministically project reusable blocks into `{ block_type, variant, props, cta, assets, metadata, version }` without changing current render/save/editor authority. CTA-capable and asset-capable fields are now explicitly sectioned at registry-definition level for the reusable lane. Boundaries preserved: homepage runtime untouched, catch-all runtime untouched, no multilingual runtime, no nested-layout runtime, no AI-generation runtime, and no Rule 16 ordering change. Final validation: post-change `npm run build` passed on May 24, 2026; archived preview-token render for `/pages/bima-sakhi-yojana` remained `200`; preview HTML retained hero/FAQ content, `/apply` continuity, and `0` `noise.png` references. Authoritative artifacts: `docs/audits/REUSABLE-RUNTIME-SURVIVABILITY.md`, `docs/audits/REUSABLE-ADMIN-FLOW-AUDIT.md`, `docs/audits/REGISTRY-GOVERNANCE-AUDIT.md`, `docs/audits/CTA-ASSET-GOVERNANCE-AUDIT.md`, `docs/tests/REUSABLE-RUNTIME-SURVIVABILITY-VALIDATION.md`, `docs/tests/HYDRATION-ROLLBACK-VALIDATION.md`, and `docs/tests/REGISTRY-RUNTIME-VALIDATION.md`.
| **REUSABLE RUNTIME FOUNDATION STATUS (May 24, 2026):** FIRST BOUNDED REUSABLE-RUNTIME FOUNDATION LAYER IMPLEMENTED LOCALLY, VALIDATED. The existing `custom_pages + page_blocks + page_versions` lane now has a tracked canonical block-contract registry (`lib/blocks/registry.js`) that normalizes reusable block payloads across public render, admin GET, admin save, and editor palette paths. `components/blocks/PageBlocks.jsx` now exposes governed hero, CTA, FAQ, testimonial, authority, media, and download primitives using normalized payloads; `app/api/admin/pages/[id]/route.js` preserves Rule 16 rollback-safe persistence while normalizing block payloads before write; `features/admin/pages/PageEditorContent.jsx` now derives palette and field controls from the same registry; and `components/ui/SmartCTA.jsx` now accepts bounded style/event inputs for reusable CTA continuity. Boundaries preserved: homepage runtime remains untouched, catch-all precedence remains untouched, metadata/SEO authority remains route-owned, App Router precedence remains highest-authoritative, and no drag-drop or unrestricted visual builder was introduced. Final validation: `npm run build` completed successfully on May 24, 2026 after the slice was integrated. Authoritative artifacts: `docs/audits/REUSABLE-RUNTIME-FOUNDATION.md` and `docs/tests/REUSABLE-RUNTIME-VALIDATION.md`.
| **IMPLEMENTATION HARDENING STATUS (May 23, 2026):** FIRST CONTROLLED GENERATED-RUNTIME IMPLEMENTATION HARDENING SLICE COMPLETE IN LOCAL MODE, VALIDATED. `app/[...slug]/page.js` now hardens deterministic generated execution by resolving `full_slug` before `page_slug` fallback, performing bounded linked-draft and version-snapshot recovery when `location_content` is incomplete, normalizing generated payloads before client hydration, and aligning breadcrumb/runtime slug rendering with the effective generated route path. Runtime validation confirmed: local production build PASS, generated runtime route `/test-route` returned `200`, explicit static route `/about` returned `200`, and orphan route `/test_audit_draft-1777869359401` remained `404` because no live content, linked draft, or version snapshot existed. Boundaries preserved: runtime continuity remains highest-authoritative, App Router precedence remains unchanged, catch-all precedence remains unchanged, no runtime renderer ownership was widened, and orphan rows were not promoted into fake runtime lineage. Authoritative artifacts: `docs/audits/IMPLEMENTATION-HARDENING.md` and `docs/tests/IMPLEMENTATION-RUNTIME-VALIDATION.md`.
| **AUTHORITY DEPLOYMENT BOUNDARY (May 26, 2026):** PAGE AUTHORITY UNIFICATION, STATIC AUTHORITY INTEGRATION, EDITABLE AUTHORITY ENABLEMENT, STRUCTURED CONTENT AUTHORITY, LINEAGE RECOVERY HARDENING, AND THE IMPLEMENTATION-HARDENING / CATCH-ALL RESOLVER SLICES remain bounded local or separate-baseline work unless independently closed. By contrast, the reusable `/pages` lane now includes the deployed reusable-runtime foundation, canonical contract evolution, persistence convergence, active runtime optimization, cross-layer survivability, and route-owned invalidation slices live on `af7d7c3`. Catch-all authority remains explicitly excluded from this deployed boundary.
| **DOCUMENTATION CONVERGENCE DEPLOYMENT STATUS (May 22, 2026):** DEPLOYED TO REPOSITORY HISTORY ON `main` AS COMMIT `81bf6db` (`docs: converge architecture and validation history`). CCC, INDEX, SHOS hardening chronology, bounded governance phases, RC-FR recovery records, and structured/editable authority validation artifacts are now preserved in repository continuity as documentation-only history. This cycle did not deploy runtime code, helper code, or any new production authority surface.
| **PHASE-8 STATUS (May 19, 2026):** BOUNDED GOVERNANCE-CENTRALIZATION FOUNDATION - Complete (planning-only bounded cycle, no activation). This cycle performed no unrestricted AI activation, no provider switching activation, no autonomous failover activation, no queue-wide orchestration, no workflow automation rollout, no governance reconstruction rollout, and no runtime authority expansion. Governance-centralization foundation was reconstructed and classified across five ownership domains: (1) provider governance ownership, (2) workflow governance ownership, (3) CRM and multi-site governance ownership continuity, (4) delivery governance durability ownership, (5) telemetry and observability governance ownership. Four mandatory final matrices were produced: provider governance matrix, workflow ownership matrix, CRM-multi-site matrix, and telemetry governance matrix. Final state remains intentionally bounded: provider fallback ownership lane = AUTHORITY_FRAGILE, workflow edge ownership lane = PARTIALLY_DURABLE, CRM/multi-site governance lane = PARTIALLY_ISOLATED, telemetry governance lane = PARTIALLY_SAFE. Hard stop preserved: no orchestration rollout, no workflow-engine activation, no authority widening, no runtime mutation activation.
| **PHASE-5 STATUS (May 19, 2026):** TARGETED OPERATIONAL HARDENING - Complete (bounded hardening cycle without runtime expansion). This cycle performed no unrestricted AI activation, no provider switching, no autonomous failover, no queue-wide orchestration, no governance redesign, and no runtime authority expansion. Operational durability hardening established the FIRST operationally hardened bounded runtime core: explicit durability classifications produced for all telemetry surfaces, three final matrices constructed (telemetry durability, queue truth, observability durability), five implementation-ready hardening targets identified, and bounded survivability verified across rollback/replay/degraded-state scenarios. Truthful classifications: event_store=OPERATIONALLY_HARDENED (durable write-ahead log, no silent degradation), retry_escalation=OPERATIONALLY_HARDENED (durable visibility through event_store), queue_truth=OPERATIONALLY_HARDENED (survives all scenarios), dead_letters=OPERATIONALLY_HARDENED (durable escalation), delivery_truth=PARTIALLY_HARDENED (graceful degradation when table unavailable), observability_logging=PARTIALLY_HARDENED (non-blocking writes, silent failures possible), system_health=PARTIALLY_HARDENED (depends on upstream telemetry), logger=INTENTIONALLY_BOUNDED (failures completely silent by design). Identified hardening targets: (1) delivery metrics unavailability visibility, (2) logger health degradation visibility, (3) replay attempt fingerprint durability, (4) deployment/rollback event markers, (5) stuck event detection cron dependency tracking. Replay ambiguity and observability silent degradation remain explicitly REQUIRES_HARDENING. Deployment continuity verified, rollback continuity preserved, all 20 hard stops maintained. Hard stop preserved: no runtime authority expansion, no unrestricted AI activation, no provider failover activation, no queue-wide orchestration.
| **PHASE-4 STATUS (May 18, 2026):** CONTROLLED RUNTIME TELEMETRY VALIDATION - Complete (validation-only observability cycle). This cycle performed no unrestricted AI activation, no provider switching, no autonomous failover, no queue-wide orchestration, no governance redesign, and no runtime authority expansion. Runtime-truth validation established the FIRST bounded rollback-safe telemetry baseline: system health aggregation is authoritative, retry escalation truth is durable through event_store plus dead-letter state, AI execution remains bounded behind ai_enabled/pagegen_enabled gates, provider degradation remains partially visible through vendor health snapshots plus bounded live probe paths, and rollback continuity remains operationally preserved. Truthful limits are explicit: queue delivery telemetry can still degrade softly if external_delivery_logs truth becomes unavailable, central observability writes remain intentionally non-blocking and can fail without stopping runtime, feature-flag history is only partially observable, rollback/deployment transitions are not yet durably emitted as first-class telemetry events, and aggressive Windows clean-rebuild loops exposed a transient `.next` manifest/cache race so clean-loop determinism is not yet trusted. Final surface authorization: retry telemetry = TRUSTED_OBSERVABLE; runtime/admin/AI/provider/rollback telemetry = PARTIALLY_TRUSTED; queue telemetry and observability substrate = REQUIRES_HARDENING. Hard stop preserved: no runtime authority expansion, no unrestricted AI activation, no provider failover activation, no queue-wide orchestration.
| **PHASE-3 STATUS (May 18, 2026):** BOUNDED DEPLOYMENT HARDENING - Complete (validation-only hardening). This cycle performed no unrestricted AI activation, no provider switching, no autonomous failover, no queue-wide orchestration, no governance redesign, and no runtime authority expansion. Runtime-truth hardening validated: (1) 3 untracked runtime-critical libs verified DEPLOYMENT_SAFE — all imports resolve to tracked files, all callers double-gated; (2) 6 trusted runtime groups constructed (RUNTIME_APIS, CORE_LIBRARIES, ADMIN_RUNTIME, PUBLIC_ROUTING, ADMIN_COMPONENTS, UNTRACKED_RUNTIME_CRITICAL); (3) 10 deployment continuity tests executed — BUILD_GREEN, DETERMINISTIC, FRESH_CLONE, ROLLBACK_SCENARIO, CACHE_INVALIDATION, CREDENTIAL_RELOAD, RUNTIME_RESTART, IMPORT_RESOLUTION, GOVERNANCE_ISOLATION, AI_GATING_INTEGRITY — all PASS or EXPECTED_PASS_WITH_ENV; (4) safe deployment authorization matrix produced for all 52 surfaces; (5) deployment discipline confirmed — zero governance imports in runtime code, zero forensic imports in runtime code, all AI execution gated. ROLLBACK BOUNDARY ESTABLISHED: current HEAD (70d3e4e+) is safe; HEAD~1 has CCC syntax error — DO NOT REVERT past current HEAD. Non-fatal anomalies documented: ENOENT `_not-found.nft.json` (Next.js 15 artifact), `jose` Edge Runtime warnings (pre-existing). Single remaining blocker: commit 3 untracked libs before Vercel push. Artifacts: PHASE3-BOUNDED-DEPLOYMENT.md, PHASE3-DEPLOYMENT-CONTINUITY.md, PHASE3-DEPLOYMENT-AUTHORIZATION.md. Hard stop preserved: no runtime authority expansion, no AI provider expansion, no queue expansion, no autonomous failover.
| **PHASE-2 STATUS (May 18, 2026):** DEPLOYMENT TOPOLOGY CLASSIFICATION - Complete (validation-only audit). This cycle classified all 62 repository surfaces (34 modified tracked + 28 untracked) into deterministic deployment categories: DEPLOYABLE_RUNTIME (25 tracked files), GOVERNANCE_ONLY (2 tracked docs + 18 untracked docs), RUNTIME_CRITICAL_UNTRACKED (3 files requiring commit), LOCAL_ONLY (4 forensic scripts). Build validation: PASS (exit code 0). No runtime mutations, no AI activations, no governance redesign. All governance gates (ai_enabled, pagegen_enabled, safe_mode) verified and intact. Zero unknown deployment surfaces; zero mixed topology; zero accidental deploy surfaces. Artifacts: docs/audits/PHASE2-DEPLOYMENT-TOPOLOGY.md, docs/tests/PHASE2-REPOSITORY-CLASSIFICATION.md. Hard stop preserved: no runtime authority expansion, no AI provider expansion, no queue expansion, no autonomous failover.
| **RC-FR11 STATUS (May 17, 2026):** COORDINATED DEGRADATION CONTAINMENT - Complete (bounded reconstruction only). This cycle performed no unrestricted AI activation, no live provider switching, no autonomous failover, no queue-wide AI routing activation, no resolver restoration, no autonomous publishing, no SHOS widening, no rollback redesign, and no observability redesign. Runtime-truth reconstruction established the first implementation-ready coordinated degradation containment map for retry/fallback/replay amplification containment, replay authority containment, desynchronization cascade containment, streaming interruption containment, and degraded observability containment. Remaining limits are explicit: retry/fallback amplification containment remains high risk under escalation, replay survivability remains partially authoritative, desynchronization containment remains partially fragmented, and streaming interruption containment remains fragile. Final RC-FR11 authorization: CONTAINMENT_REQUIRES_MORE_RECONSTRUCTION.
| **RC-FR10 STATUS (May 17, 2026):** DEGRADED-STATE COORDINATION HARDENING - Complete (bounded reconstruction only). This cycle performed no unrestricted AI activation, no live provider switching, no autonomous failover, no queue-wide AI routing activation, no resolver restoration, no autonomous publishing, no SHOS widening, no rollback redesign, and no observability redesign. Runtime-truth reconstruction established the first implementation-ready degraded-state coordination hardening map for retry/fallback storm survivability, queue desynchronization survivability, replay authority modeling, streaming/envelope degraded continuity, and degraded observability continuity. Remaining limits are explicit: fallback coordination remains fragmented under degraded states, retry/fallback amplification survivability remains fragile, queue desynchronization survivability remains partial, and envelope normalization remains partially route-local. Final RC-FR10 authorization: HARDENING_REQUIRES_MORE_RECONSTRUCTION.
| **RC-FR8 STATUS (May 17, 2026):** AUTHORITATIVE AI EXECUTION COORDINATOR - Complete (bounded reconstruction only). This cycle performed no unrestricted AI activation, no live provider switching, no autonomous failover, no queue-wide AI routing activation, no resolver restoration, no autonomous publishing, no SHOS widening, no rollback redesign, and no observability redesign. Runtime-truth reconstruction established the first implementation-ready execution-coordinator authority map for provider resolution, task-policy enforcement, retry/fallback coordination, execution-envelope coordination, drift visibility, and authority-typed observability emission. Remaining limits are explicit: execution coordination remains partially fragmented, retry and fallback authority remain partially provider-coupled, and drift visibility is only partially authoritative in desynchronization and degradation windows. Final RC-FR8 authorization: COORDINATION_REQUIRES_MORE_RECONSTRUCTION.
| **RC-FR7 STATUS (May 16, 2026):** RUNTIME-AUTHORITATIVE GOVERNANCE LANE HARDENING - Complete (bounded reconstruction only). This cycle performed no unrestricted AI activation, no live provider switching, no autonomous provider failover, no queue-wide routing activation, no resolver restoration, no autonomous publishing, no SHOS widening, no rollback redesign, and no observability redesign. Runtime-truth reconstruction established the first implementation-ready governance-enforcement lane for authoritative provider/task/gate/retry resolution modeling, drift authority modeling, symmetry hardening targets, and execution-observability authority distinctions. Remaining limits are explicit: selected admin AI routes still require symmetry closure, retry semantics remain partially provider-shaped, and drift detection remains partially authoritative. Final RC-FR7 authorization: HARDENING_REQUIRES_MORE_RECONSTRUCTION.
| **RC-FR6 STATUS (May 16, 2026):** GOVERNANCE-SAFE PROVIDER REGISTRY AND TASK-POLICY FOUNDATION - Complete (bounded reconstruction only). This cycle performed no unrestricted AI activation, no provider switching/failover activation, no queue-wide runtime cutover, no autonomous publishing, no SHOS widening, no rollback redesign, and no observability redesign. Runtime-truth reconstruction confirms active execution remains provider-coupled while provider registry and task policy remain non-authoritative for live execution routing. Primitive survivability and authority mapping were reconstructed; trust boundaries remain partial because route-level gate symmetry is incomplete and failure governance remains provider-coupled in active paths. Drift classes were explicitly captured: governance drift, authority drift, provider drift, and policy drift. Final RC-FR6 classifications: provider registry foundation = FOUNDATION_PRESENT_BUT_NON_AUTHORITATIVE, task policy foundation = LIMITED_FOUNDATION_ONLY, symmetry = SYMMETRY_INCOMPLETE, drift = DRIFT_PRESENT_REQUIRES_HARDENING, final authorization = FOUNDATION_REQUIRES_MORE_RECONSTRUCTION.
| **RC-FR5 STATUS (May 16, 2026):** PROVIDER-NEUTRAL AI EXECUTION FABRIC - Complete (bounded reconstruction only). This cycle performed no unrestricted AI activation, no live provider failover, no autonomous provider switching, no queue-wide routing activation, no resolver restoration, no autonomous publishing, no SHOS widening, no rollback redesign, and no observability redesign. Runtime-truth reconstruction confirms active execution remains provider-coupled: Gemini still anchors runtime generation semantics, route-level parsing remains non-normalized, and retry/quota/error handling remains Gemini-coupled in the active path. OpenAI remains physically present but non-authoritative at runtime. Provider/model/task runtime registries remain non-authoritative and governance-safe execution is still incomplete for live cutover. Additional governance inconsistency risk was observed: some admin AI routes currently execute without explicit in-route `ai_enabled` gate enforcement, leaving control-plane symmetry incomplete. Final RC-FR5 classifications: provider adapter contracts = PARTIALLY_COUPLED with GEMINI_COUPLED/HIGH_RISK domains, execution semantics = PROVIDER_COUPLED, provider failure semantics = PARTIALLY_SURVIVABLE, trust boundaries = PARTIALLY_TRUSTED with FABRIC_FRAGILE domains, final fabric authorization = FABRIC_REQUIRES_MORE_RECONSTRUCTION.
| **RC-FR4 STATUS (May 16, 2026):** AI GOVERNANCE EXECUTION FOUNDATION - Complete (bounded reconstruction only). This cycle performed no unrestricted AI activation, no autonomous publishing, no resolver-chain restoration, no queue-wide activation widening, no SHOS widening, no rollback redesign, and no observability redesign. Provider-governance reconstruction confirms that active execution remains Gemini-coupled (`lib/ai/generateContent.js`, `lib/ai/index.js`, pagegen/admin AI routes), while OpenAI remains physically present but runtime-non-authoritative. Governance controls remain partial: global AI gating and control-plane flags exist, but provider registry, runtime-authoritative model registry, and task-routing execution fabric are still missing. Provider-specific retry/response/error assumptions remain embedded in active paths. Authority topology in RC-FR4 remains preserved because this cycle was analysis+documentation only. Final RC-FR4 classifications: provider governance = PARTIALLY_IMPLEMENTED with GOVERNANCE_FRAGILE risk envelope, task routing = LIMITED_FOUNDATION_ONLY, final foundation authorization = FOUNDATION_REQUIRES_MORE_RECONSTRUCTION.
| **RC-FR3 STATUS (May 16, 2026):** AI CONTROL PLANE & PROMPT ENGINE RECONSTRUCTION - Complete (bounded analysis only). This cycle performed no unrestricted AI activation, no resolver-chain restoration, no SHOS widening, no schema/migration change, no rollback redesign, and no observability redesign. Runtime-truth reconstruction across AI routes, pagegen worker, prompt engine, flags, and system-health probes confirms that active generation paths remain directly Gemini-coupled and provider abstraction is incomplete. `lib/ai/promptEngine.js` remains local-only in current git-state truth, while several AI execution surfaces remain locally modified and not deploy-converged. Governance gates (`ai_enabled`, `pagegen_enabled`, `safe_mode`, `ai_prompt_templates_enabled`) are present, but provider-level switching, provider-neutral response adapters, and provider-failure survivability controls remain missing foundations. Final RC-FR3 classifications: AI recovery = AI_RECOVERY_REQUIRES_MORE_RECONSTRUCTION, prompt-engine = LIMITED_AI_RECOVERY_ONLY, governance = GOVERNANCE_FRAGILE. FR-C activation lane remains deferred; FR-D resolver lane remains deferred.
| **RC-FR2 STATUS (May 16, 2026):** CONTROLLED FEATURE RECOVERY EXECUTION - Complete for the authorized bounded groups. This cycle performed no unrestricted AI activation, no resolver-chain restoration, no SHOS widening, no schema/migration change, and no rollback redesign. FR-A restored bounded admin/media polish and operator-facing UI wrappers in isolated commits (`da31455c70889518d1c2ef4f78cb105e64e6a433`). FR-B restored the CMS/CCC inventory shell plus blog/resources wrappers and CMS structure closure in an isolated bounded commit (`1534ad038ac89e990d032e5b27722d21e69ecf24`). Content inventory was reduced to a pure inventory shell to avoid carrying AI-generation recovery into this cycle. Final RC-FR2 classifications: recovery = RECOVERY_SUCCESSFUL_WITH_LIMITATIONS, recovery trust = PARTIALLY_TRUSTED, deployment coherence = TOPOLOGY_SAFE for FR-B and preserved for FR-A. FR-C and FR-D remain intentionally deferred.
| **RC-FR1 STATUS (May 16, 2026):** FEATURE RECOVERY & DEPLOYMENT RECONSTRUCTION - Complete (analysis scope only). This cycle performed no deployment, no runtime mutation, no schema/migration changes, no SHOS widening, and no governance redesign. Remaining 49 runtime/admin/product local paths were reconstructed by capability class: DEPLOYED_AND_ACTIVE, LOCAL_ONLY, PARTIALLY_WIRED, FEATURE_GATED, SUPPRESSED, ORPHANED, and RECOVERABLE. Critical local-only dependency closures remain undeployed (`lib/ai/promptEngine.js`, `lib/cms/resolveRoute.js`, `lib/cms/resolveCmsRoute.js`, `features/admin/content/ContentInventoryContent.jsx`, `app/api/admin/cms/structure/route.js`). AI and generation paths remain explicitly gated (`ai_enabled`, `pagegen_enabled`, `ai_prompt_templates_enabled`, `bulk_generation_enabled`), and SHOS mutation authority remains suppression-sensitive in the current safety era. Final RC-FR1 classifications: feature recovery = LIMITED_RECOVERY_ONLY, admin restoration = LIMITED_RECOVERY_ONLY, deployment reconstruction = RECOVERY_REQUIRES_MORE_RECONSTRUCTION.
| **RC-G2 STATUS (May 16, 2026):** PHYSICAL GIT SEGREGATION & CLEAN DEPLOY BASELINE EXECUTION - Complete (execution scope only). This cycle performed no deployment, no runtime mutation, no feature activation, no SHOS widening, and no AI recovery. Physical segregation was executed in two isolated commits: docs-only surface (`docs/**`) in `710ca31de3ffab701140d9b94a0a173087b276a6`, and generated-artifact cleanup (`.gitignore`, `next-build.log`, `scripts/audit/results/**`) in `c6d35c05c0ce3080c35467fc874528148339c091`. Mixed dirty state was materially reduced from 271 to 49 paths. Remaining dirty surfaces are now constrained to runtime/admin review-required lanes plus local-only scripts. Final RC-G2 classifications: git restoration = MOSTLY_RESTORED_WITH_LIMITATIONS, deployment topology trust = PARTIALLY_RESTORED, rollback-boundary trust = PARTIALLY_RESTORED.
| **RC-G1 STATUS (May 16, 2026):** GIT SEGREGATION & DEPLOYMENT BASELINE RESTORATION - Complete (read-only git-state normalization cycle). This cycle performed no deployment, no runtime mutation, no feature activation, and no architecture redesign. Authoritative git surface reconstruction found materially dirty mixed state (parsed baseline: `total_dirty_count=271`, `tracked_modified_count=131`, `untracked_count=140`, `deleted_count=89`) but full path classification was achieved (`unknown_other=0`). Deterministic deployment model was reconstructed with explicit classes: SAFE_DEPLOYABLE, ATOMIC_ONLY, LOCAL_ONLY, DO_NOT_DEPLOY, REVIEW_REQUIRED. Atomic rollback/deploy coupling was reasserted for auth pair (`middleware.js` + `lib/auth/withAdminAuth.js`) and SHOS control/canary runtime surface. Generated logs/result artifacts were explicitly isolated as non-deployable. Final classifications: git-state = DIRTY_MIXED_SURFACES_WITH_ZERO_UNKNOWN_PATHS, deployment-baseline = PARTIALLY_SEGREGATED, rollback-clarity = PARTIALLY_RESTORED, authority-boundary = PARTIALLY_RESTORED, final git authorization = REQUIRES_MANUAL_GIT_INTERVENTION. Hard stop preserved: no SHOS widening, no AI activation, no runtime expansion. Artifacts: docs/audits/RC-G1-GIT-SURFACE-BASELINE.md, docs/audits/RC-G1-GIT-SEGREGATION.md, docs/tests/RC-G1-DEPLOYMENT-BASELINE.md, docs/tests/RC-G1-ROLLBACK-CLARITY.md.
| **RC-3A STATUS (May 16, 2026):** CONTROLLED SHOS OPERATIONAL EXPANSION AUTHORIZATION - Complete (read-only analysis cycle). This cycle performed no deployment expansion, no feature activation, no schema/migration changes, no auth/middleware redesign, and no suppression/rollback/observability redesign. Post-convergence operational baseline remained stable across uncached sequential checks at `version=09b8e23`: suppression stayed active (`suppressed=true`, `reverted=0`), protected observability continuity remained `200`, queue stayed `failed=0`, DLQ endpoint total stayed `0`, and delivery remained stable (`delivery_failures_recent=0`, `delivery_stuck_count=0`). Residual bounded degraded signals persisted (`unacknowledged_escalations`, `historical_dead_letters:2`) and consistency friction remained visible (`matches_health_dlq_total=false`). Blast-radius survivability classification: LIMITED_TO_ADMIN_ONLY_CONTROLLED_EXPANSION. Trust boundary conclusion: TRUST_DEGRADED_BEYOND_ADMIN_ONLY. Operational scalability: CONDITIONALLY_SCALABLE. Rollback scalability: CONDITIONALLY_SCALABLE_ADMIN_ONLY. Observability scalability: PARTIALLY_SCALABLE_WITH_OVERLOAD_RISK. Final expansion authorization classification: LIMITED_EXPANSION_ONLY. Hard stop preserved: no automatic widening to cross-runtime or global-runtime surfaces. Artifacts: docs/audits/RC-3A-OPERATIONAL-BASELINE.md, docs/audits/RC-3A-EXPANSION-AUTHORIZATION.md, docs/tests/RC-3A-BLAST-RADIUS-VALIDATION.md, docs/tests/RC-3A-ROLLBACK-SCALABILITY.md.
| **RC-2L STATUS (May 16, 2026):** POST-CANARY CONVERGENCE AUTHORIZATION - Complete (read-only verification cycle). This cycle performed no deployment expansion, no feature/surface expansion, no schema/migration changes, no auth/middleware redesign, and no runtime mutation. Post-canary durability checks across protected authenticated surfaces showed stable suppression (`/api/admin/system/shos` = `200`, `auto_reverts.suppressed=true`, `reverted=0`), stable queue/DLQ/delivery channels (`queue failed=0`, `dlq total=0`, `delivery_failures_recent=0`, `delivery_stuck_count=0`), protected auth continuity (`/admin/system` and protected APIs remained `200`), and runtime version continuity (`/api/status` + `/api/admin/system/health` both reporting `07607b5`). Delayed-regression classification: NO_DELAYED_REGRESSION in sampled RC-2L window. Observability durability classification: PARTIALLY_TRUSTWORTHY due persistent residual degraded signatures (`unacknowledged_escalations`, `historical_dead_letters:2`) and consistency friction indicator (`matches_health_dlq_total=false`). Suppression durability classification: SUPPRESSION_DURABLE_WITH_LIMITATIONS. Human-governance survivability classification: SURVIVABLE_WITH_LIMITATIONS. Final convergence classification: CONVERGENCE_CONFIRMED_WITH_LIMITATIONS. Hard stop preserved: no rollout widening authorized in RC-2L. Artifacts: docs/audits/RC-2L-POST-CANARY-BASELINE.md, docs/audits/RC-2L-POST-CANARY-CONVERGENCE.md, docs/tests/RC-2L-DELAYED-REGRESSION-VALIDATION.md, docs/tests/RC-2L-OBSERVABILITY-DURABILITY.md.
...
| **System State:** SHOS_LIVE_AUTHORITY_CONFIRMED
| **Operational Durability:** PHASE-5 runtime durability baseline preserved; PHASE-8 governance ownership topology remains bounded; production now runs the hardened SHOS authority layer with explicit operational, forensic, escalation, replay, dead-letter, and operator-visible surfaces
| **Next Active Phase:** Post-deploy observation only. Keep orchestration rollout, workflow automation activation, multi-provider live switching, autonomous failover, and all runtime authority widening deferred.
...
> **SHOS OBSERVABILITY RECONCILIATION (May 20, 2026):** Reconciliation-only cycle completed. No code, DB state, or runtime control was changed. At that pre-hardening baseline, live SHOS remained operational but `DEGRADED` because stale unacknowledged P0 escalations and historical dead-letter residue were still influencing the summary verdict while the top-level operator recovery lists remained only partially aligned. The authoritative classification for that baseline was `OBSERVABILITY_DRIFT_PRESENT`, with boundary `PARTIALLY_BOUNDED`, operator trust `PARTIALLY_TRUSTED`, and continuity `PARTIALLY_DURABLE`. This paragraph is preserved as historical pre-deployment context only; current live truth is the deployed SHOS hardening validation recorded on May 21, 2026 below. Local SHOS API remained healthy; local `/admin/ccc` cold-start instability was a development ergonomics issue, not production runtime authority. See [SHOS-RECONCILIATION](audits/SHOS-RECONCILIATION.md), [SHOS-OBSERVABILITY-ALIGNMENT](tests/SHOS-OBSERVABILITY-ALIGNMENT.md), and [audit-2026-05-20-shos-live-local-problem-audit](audits/audit-2026-05-20-shos-live-local-problem-audit.md).

> **SHOS HARDENING IMPLEMENTATION (May 20, 2026):** Implementation-hardening cycle completed locally. Code changes were limited to summary shaping, authority classification, and operator visibility alignment. The hardened layer now separates operational and forensic health, splits escalation visibility into live/stale/acknowledged lanes, promotes replay visibility to a first-class operator lane, and preserves historical dead-letter residue explicitly. Local validation passed for login, `/api/admin/system/shos`, and `/admin/system` reachability. `npm run build` compiled successfully, then failed on local `.next` artifact ENOENT variants (`pages-manifest.json` and `_not-found/page.js.nft.json` across separate runs) already classified as a local Next.js artifact anomaly rather than a new SHOS compile regression. This record is the final local pre-deployment hardening baseline only. No deployment and no production mutation occurred in this cycle. See [SHOS-HARDENING-IMPLEMENTATION](audits/SHOS-HARDENING-IMPLEMENTATION.md) and [SHOS-AUTHORITY-VALIDATION](tests/SHOS-AUTHORITY-VALIDATION.md).

> **SHOS LIVE DEPLOYMENT VALIDATION (May 21, 2026):** Guarded deployment cycle completed. Because the repo working tree remained broadly dirty, production deployment was executed from an isolated detached worktree containing only the three hardened SHOS runtime files: `lib/system/systemHealth.js`, `lib/system/shos.js`, and `features/admin/system/ShosControlCenter.jsx`. Candidate deployment `dpl_nZshvQWtjGXGBbJNHvvnzkYy4Lky` was first built in production target mode without domain promotion, then promoted live with explicit team scope after health proof. Live validation on `https://bimasakhi.com` confirmed: `public_overall_health=HEALTHY`, `authority_model_present=true`, `live_escalations=0`, `stale_escalations=2`, `acknowledged_escalations=6`, `historical_incidents_count=1`, `replay_item_count=0`, `stuck_events_count=0`, `active_dlq_alignment=true`, `forensic_dlq_alignment=true`, and `/admin/system=200`. Live authority model now explicitly classifies `overall_health` and `operational_summary` as `LIVE_OPERATIONAL`, `forensic_summary` as `FORENSIC`, escalation visibility as `ESCALATION`, replay visibility as `REPLAY`, dead-letter visibility as `DEAD_LETTER`, and operator lists as `OPERATOR_VISIBLE`. Previous production deployment `dpl_3Y8fu2wsELvYb3XN3aXYxJ58jfdb` remained `Ready`, preserving deterministic rollback continuity. Remaining limitation: non-browser HTML heading checks on `/admin/system` remained inconclusive because the probe sees streamed shell output, so operator trust continuity is classified `PARTIALLY_DURABLE` rather than full browser-proven. See [SHOS-LIVE-DEPLOYMENT](audits/SHOS-LIVE-DEPLOYMENT.md) and [SHOS-PRODUCTION-VALIDATION](tests/SHOS-PRODUCTION-VALIDATION.md).
...
7. [Engine 3: Page Templates + Layouts + Images](#7-engine-3-page-templates--layouts--images)
8. [Engine 4: Publish + Distribution + Sitemap](#8-engine-4-publish--distribution--sitemap)
9. [Download-as-Lead-Magnet System](#9-download-as-lead-magnet-system)
10. [Form Guidance + Agent Customization Service](#10-form-guidance--agent-customization-service)
11. [Zoho One Integration (45 Apps, 5-10 Relevant)](#11-zoho-one-integration)
12. [Google Business Profile Strategy](#12-google-business-profile-strategy)
13. [Multi-City + Pincode Micro-Local Engine](#13-multi-city--pincode-micro-local-engine)
14. [Anti-Spam + Google Safety System](#14-anti-spam--google-safety-system)
15. [New Database Schema](#15-new-database-schema)
...
> *"Fast nahi banna hai. Stable + scalable banna hai."*  
> *"System banana engineering hai. System ko stable rakhna leadership hai."*  
> *"System tab powerful hota hai jab uske rules strong hote hain, features nahi."*
...
CEO control means:
  ...
  - ON/OFF toggle where applicable
  - Configuration change without touching code
  - Visibility into what's happening (logs, status, metrics)

Feature-wise CEO Control Requirements:
  | Feature Area    | CEO Must Be Able To                          |
  |-----------------|----------------------------------------------|
  | Content         | Create, edit, delete, approve, reject pages   |
  | Geo             | Add city, locality, pincode from admin        |
  | Funnels         | Toggle ON/OFF, configure lead capture         |
  | AI              | Change model, edit prompts, set limits        |
  | Alerts          | Select channels, set thresholds, test alerts  |
  | Navigation      | Edit menus, add/remove items, reorder         |
  | Media           | Upload, organize, assign to content           |
  | Users           | Create users, assign roles, manage access     |
...
Every admin page MUST provide:
  ...
  |-----------------|------------------------------------------|
  | View            | See all data with filters + search        |
  | Create          | Add new items from UI (not SQL/code)      |
  | Edit            | Modify existing items inline              |
  | Delete          | Remove with confirmation + soft delete    |
  | Bulk Actions    | Select multiple + batch operation         |
  | Export          | Download data as CSV/PDF                  |
  | Filter          | By date, status, type, search term        |
  | Sort            | By any column                             |
  | Pagination      | Never load all data at once               |

  Admin page without Create = Read-only dashboard = NOT complete.
  CEO's exact words: "Kya mere pass koi bhi power nahi hai?"
  That question should NEVER arise again.
```
...
**Why:** Parallel work on multiple modules creates entangled code, unclear ownership, half-finished states, and untestable deployments. Speed comes from depth of focus, not breadth of activity.

**In practice for this project:**

| Phase | One Module | Sub-modules |
|-------|-----------|------------|
| Phase 1 | Rendering Gap | (1a) catch-all route, (1b) page template, (1c) metadata, (1d) schema markup |
| Phase 2 | Draft System | (2a) DB migration, (2b) worker writes to drafts, (2c) admin list page, (2d) editor |
...
**Four mandatory proofs per feature:**
...
**For SEO pages specifically, also verify:**
- `<title>` tag matches `meta_title` from DB
- `<meta name="description">` matches `meta_description`
- `<h1>` is the `hero_headline`
...
Bimasakhi.com is live. It has real users, real leads, real revenue. Every change must be backward compatible.
...
- What if the DB query returns 0 rows?
- What if it returns more rows than expected?

**Failure Cases:**
- What if the AI API (Gemini) is down or rate-limited?
- What if QStash delivery fails or the webhook times out?
- What if the DB write fails halfway through (partial write)?
- What if two workers process the same queue job simultaneously?

**Data Flow:**
- Exact sequence of operations (numbered steps)
- Which table is written first, which is written second?
- Are there any foreign key dependencies that require a specific order?

**API Contract (Rule 7 below covers this in detail)**

If any of these questions cannot be answered before coding starts, the design is not complete yet.
...
**Contract format for every API endpoint:**
...
INPUT:
  ...
  Auth: required (admin JWT) | QStash signature | public
  Body: { field: type, required: yes/no, validation: rule }
...
INPUT:
  Method: POST
  Auth: QStash signature (verified via QSTASH_CURRENT_SIGNING_KEY)
  Body: { queueId: UUID (required) }

OUTPUT (success):
  Status: 200
  Body: { success: true, draftId: UUID, slug: string, wordCount: number }

OUTPUT (errors):
  400: missing queueId
  404: queueId not found in generation_queue
  409: slug already exists (duplicate)
...
SIDE EFFECTS:
  ...
  - content_fingerprints: 1 new row (SHA256)
  - observability_logs: success or failure entry
```

This contract is written BEFORE the code. The code implements the contract. Tests verify the contract.
...
|-----------|-------------|---------------|
| **Happy Flow** | Normal working scenario with valid inputs | Expected output returned, correct DB state |
| **Edge Case** | Empty data, missing optional fields, boundary values | Graceful handling, no crash, correct error code |
| **Failure Case** | Simulated DB failure, API timeout, invalid auth | Correct error response, no partial write, no data corruption |
| **Load Test** | Multiple rapid requests or concurrent workers | No race conditions, no duplicate writes, rate limits enforced |

**How we test in this project:**
...
```javascript
// Success
[PAGEGEN SUCCESS] queueId=abc123 slug=lic-agent-krishna-nagar draftId=def456 words=724 duration=3210ms

// Failure
[PAGEGEN FAILED] queueId=abc123 reason="Gemini API rate limited" retryable=true attempt=2/3

// Skip
[PAGEGEN SKIP] queueId=abc123 reason="duplicate slug detected" existing_draft=def789

// Warning
[PAGEGEN WARNING] queueId=abc123 word_count=380 below_threshold=400 flagged_for_review=true

// Queue state
[QUEUE EMPTY] source=admin_manual no pending pagegen jobs found
```
...
**Mandatory guardrails for this project:**

| Guardrail | Limit | Enforcement Point |
|-----------|-------|------------------|
| Daily page publish cap | 50 pages/day (total, all types) | `content_drafts` publish API |
| Daily generation cap | 200 queue entries/day | Bulk job planner |
...
This rule is the foundation of Section 20 (CEO Control Principle) and extends to every AI-powered feature in the system.

**Non-negotiable AI limits:**

| AI Can Do Autonomously | AI CANNOT Do Without Human Approval |
|-----------------------|-------------------------------------|
| Generate content draft | Publish any content to live site |
| Score content quality | Override quality thresholds |
| Generate image prompts | Buy/upload actual images |
| Suggest re-optimization | Apply re-optimization to live page |
| Generate social post drafts | Post to any social platform |
| Detect keyword gaps | Create bulk generation jobs |
| Score leads automatically | Reassign agent for a lead |
| Send pre-approved WhatsApp templates | Send custom or new WhatsApp messages |
| Trigger pre-approved email sequences | Create or modify email templates |

**The approval UI principle:** Every AI action that crosses from "suggestion" to "live effect" must have a clearly visible Approve / Edit / Reject button in the admin. No silent auto-executions.
...
**Layer 2: DB (Data)**
- Every new table: `DROP TABLE IF EXISTS` at top of migration for clean re-run in dev
- Every `ALTER TABLE`: always `ADD COLUMN IF NOT EXISTS` (never breaks if re-run)
- Before any bulk data operation: export the affected rows to a backup table first
  ```sql
  CREATE TABLE crm_leads_backup_20260417 AS SELECT * FROM crm_leads;
  ```
- To rollback DB: restore from backup table or run inverse migration

**Layer 3: Feature Flags (Live System)**
- New features wrapped in `system_config` flags
  ```javascript
...
**Do not over-document.** Obvious code needs no comment. Comments explain WHY, not WHAT.
...
Every module that calls an external API (Gemini AI, QStash, Zoho, WhatsApp Business API, GSC API) must have cost awareness built in.

**Mandatory cost tracking per module:**

| Metric | What to Track | Where |
|--------|--------------|-------|
| Per-call cost | Estimated token/API cost per single invocation | Code comment + system docs |
| Daily cost cap | Maximum daily spend for this module | `system_control_config` |
| Monthly projection | At current usage rate, what's the monthly cost? | Admin dashboard |
| Scale cost | At 10x current volume, what's the cost? | Design doc |

**Implementation requirements:**

```
1. EVERY AI call (Gemini) must track:
   - Input tokens used
   - Output tokens used  
   - Model used (flash vs pro)
   - Estimated cost per call

2. EVERY QStash call must track:
   - Messages published per day
   - Retries consumed
   - Daily/monthly message count vs plan limit

3. EVERY Zoho API call must track:
   - API credits consumed
   - Rate limit remaining
   - Daily call count vs plan limit

4. Cost dashboard in admin (/admin/analytics/costs):
   - Today's AI spend
   - Today's API calls by service
   - Monthly trend
   - Alert if daily cost exceeds threshold
```

**Cost guardrails (stored in `system_control_config`):**

| Guardrail | Default | Enforcement |
|-----------|---------|-------------|
| `ai_daily_cost_cap_usd` | $5.00 | Pagegen + social + re-optimization jobs check before calling AI |
| `ai_monthly_cost_cap_usd` | $100.00 | Monthly cost tracker alerts at 80%, blocks at 100% |
| `qstash_daily_message_cap` | 500 | Publisher checks before enqueuing |
| `zoho_daily_api_cap` | 1000 | CRM sync checks before calling |
...
| Sin | Example | Impact |
|-----|---------|--------|
| Orphan draft | `content_drafts` row created but `page_index` row failed | Admin shows draft, but page is invisible to system |
| Ghost lead | `crm_leads` row created but Zoho CRM sync failed | Lead exists locally but agent never gets notified |
| Zombie queue | `generation_queue` marked `completed` but `content_drafts` insert failed | Queue shows job done, but no draft exists |
...
For operations that write to 2+ tables:
  ...
  - OR implement compensation: if Step 2 fails, explicitly undo Step 1
...
**Pattern 3: Idempotency keys for all write operations**
```
...
No admin endpoint, no admin page, no system action should operate without these three layers.

**Layer 1: Authentication (WHO is acting?)**
...
**Layer 2: Authorization (ARE THEY ALLOWED?)**
...
**Input validation on all admin APIs:**
```
...
Every business function must be modeled from first touchpoint to final outcome. A lead form is not a lead system. An agent registration is not an agent management system. A policy sale is not a customer relationship.

**The 5 Ecosystems of an LIC Development Officer:**

```
The CEO is an LIC Development Officer (DO). His business is NOT just a website.
...
The bible (Sections 1-32) designed the **content generation + SEO machine** in extreme detail. But that machine is just the ENTRY POINT for the business. Without modeling the full ecosystems, the system will:

| Gap | Impact |
|-----|--------|
| Only captures leads, doesn't track conversion to actual agents | No ROI visibility. Are SEO leads actually becoming agents? |
| No agent pipeline management | DO manages 50+ candidates via WhatsApp. No visibility, no tracking. |
| No active agent production tracking | Cannot identify which agents need help, which are stars |
| No customer lifecycle view | Cannot track policies, renewals, cross-sell opportunities |
| No lead source ROI | Cannot compare: website leads vs referral leads vs offline leads |

**Implementation principle:**

```
For every ecosystem:
  1. Define the COMPLETE lifecycle stages
  2. Define the DATA MODEL (what's tracked at each stage)
  3. Define the ADMIN VIEWS (how the DO sees and manages it)
  4. Define the AUTOMATION (what runs automatically vs manually)
  5. Define the INTEGRATION points (Zoho CRM, WhatsApp, email)
  6. Define the METRICS (how success is measured)
...
Every phase MUST have a clear priority rank (A/B/C/D) and a dependency chain. No phase begins without knowing:
1. What must be complete BEFORE this phase can start (hard dependency)
2. What's the business impact if this phase is delayed (priority rank)
3. What's the minimum viable version of this phase (MVP scope)
...
The bible now defines 19 implementation phases + 37 sections. Without explicit priority ordering, the CTO will:
- Build Phase 12 (Intelligence Layer) before Phase 3 (Safety)
- Add advanced features to a fragile foundation
- Create impressive demos on broken infrastructure
...
Every external system (Supabase, Vercel, QStash, Zoho, Gemini, any future vendor) MUST have a written contract defining:
...
The system is "internally deterministic, externally probabilistic." Internal code is controlled, tested, predictable. External systems are uncontrolled, can fail anytime, can change APIs, can rate-limit, can have outages. Without vendor contracts, a 2-hour Supabase outage = system is completely dead with no recovery plan.
...
Every dispatched event (QStash message, DB write, API call, webhook) must be:
...
CREATE TABLE dead_letter_queue (
    ...
    -- Original event
    original_event_type TEXT NOT NULL,       -- 'pagegen', 'lead_sync', 'zoho_push', etc.
    original_payload JSONB NOT NULL,
    original_queue_id TEXT,                  -- QStash message ID
...
Every critical alert MUST have a delivery channel, retry, and escalation path:
...
ALERT SEVERITY LEVELS:
  ...
  P3 � INFO:
    ...
    Retry:      None
    Escalation: None
    Example:    "12 pages generated today. 2 below quality threshold."
```
...
CREATE TABLE alert_deliveries (
    ...
    context JSONB,                           -- Additional data for the alert
    
    -- Delivery tracking
    channels_attempted TEXT[] NOT NULL,       -- ['whatsapp','email','cliq']
    channels_delivered TEXT[] DEFAULT '{}',   -- Which ones succeeded
    ...
    -- Escalation
    escalation_level INTEGER DEFAULT 0,      -- 0=initial, 1=first escalation, 2=final
...
**CEO Morning Brief Delivery:**
```
The CEO Morning Brief (Section 38, Layer 6) is delivered via:
  1. Admin dashboard page: /admin/intelligence/brief (always available)
...
Every critical external service MUST have a fallback strategy:
...
|--------|--------|---------|----------|--------|
| API response time | < 500ms | 500ms-2s | > 2s | P2 alert at warning, P1 at critical |
| DB query time | < 100ms | 100ms-500ms | > 500ms | Log slow queries, alert at critical |
| QStash delivery delay | < 5s | 5s-30s | > 30s | P2 alert, check QStash status |
| Error rate (per hour) | < 1% | 1-5% | > 5% | P1 alert, consider safe mode |
| AI generation time | < 30s | 30s-60s | > 60s | Switch to faster model |
| Uptime (rolling 24h) | > 99.5% | 99-99.5% | < 99% | P0 alert, investigate root cause |

**System Operating Modes:**

```
MODE 1: NORMAL (Green)
  All systems operational. Full functionality.
  system_mode = 'normal'

MODE 2: DEGRADED (Yellow)
...
CREATE TABLE sla_snapshots (
    ...
    -- What
    service TEXT NOT NULL,                    -- 'supabase', 'qstash', 'gemini', 'vercel', 'zoho'
    metric TEXT NOT NULL,                    -- 'response_time', 'error_rate', 'uptime', 'queue_delay'
    
    -- Measurement
    value NUMERIC NOT NULL,                  -- The actual measured value
    ...
    -- Context
    sample_size INTEGER,                     -- How many requests in this window
    window_minutes INTEGER DEFAULT 5,        -- Measurement window
...
- Same bugs get re-investigated repeatedly
- Audit findings exist only in chat history (lost)
- Code changes have no traceable reason
- CEO asks "what happened?" and no one can answer in 5 seconds

**The Mandate:**

Every significant system action MUST be documented in `docs/` with cross-references:

| Action Type | Where It Goes | What It Contains |
|---|---|---|
| Forensic Audit | `docs/audits/audit-YYYY-MM-DD.md` | Findings, scores, evidence, pass/fail per feature |
| Bug Fix / Code Change | `docs/fixes/fix-{description}.md` | What broke, root cause, what was changed, files modified, verification |
| New Feature | `docs/features/feature-{name}.md` | What was built, why, which bible section, files created/modified |
| Incident / Outage | `docs/incidents/incident-{description}.md` | What failed, when, impact, root cause, resolution, prevention |
| Architecture Decision | `docs/decisions/decision-{topic}.md` | Context, options considered, decision made, trade-offs accepted |
| DB Migration | `docs/migrations/migration-{description}.md` | SQL executed, tables affected, before/after state, rollback SQL |

**File Template (Mandatory Fields):**

```markdown
# [Type]: [Title]

**Date:** YYYY-MM-DD
**Author:** [CTO/CEO/Agent]
**Bible Reference:** Section X, Rule Y
...
## Remaining
What's still open (if any).

## Cross-References
- Related audit: `docs/audits/...`
- Related fix: `docs/fixes/...`
- Bible section: Section X
```

**Bible Cross-Linking:**

This document (`CONTENT_COMMAND_CENTER.md`) MUST reference the docs system:
- Section 40 defines the full System Memory architecture
- Every completed phase links to its feature doc
- Every known issue links to its incident/fix doc
- The Appendix phase tracker includes doc references

**Agent Operating Rule:**

```
WHEN completing any audit, fix, feature, or incident:
  1. Create the appropriate docs/ file FIRST
  2. Cross-reference it in the bible if relevant
  3. Update the docs/INDEX.md with the new entry
  4. THEN report completion to CEO

NEVER:
  - Leave findings only in chat
  - Make code changes without a docs/fixes/ entry
  - Complete an audit without a docs/audits/ file
...
**Mandatory Channels (in priority order):**

| Priority | Channel | Use Case | Delivery Target |
|----------|---------|----------|-----------------|
| P0 | WhatsApp Business API | Critical alerts, lead notifications | CEO phone |
| P1 | Telegram Bot | System alerts, daily reports | CEO + CTO channel |
| P2 | Email | Detailed reports, weekly summaries | CEO email |
| P3 | Zoho Cliq | Team operations, internal comms | Team workspace |
...
- Cross-system search (search finds content regardless of where it lives)
- Unified status tracking (draft, pending, published, archived across all types)
- No content should exist that CEO cannot find from admin
...
**CEO Must Be Able To:**
- Add new city with configuration (state, priority, status)
- Add new locality under any city
- Import pincodes (CSV upload or individual)
- Toggle city/locality active/inactive
- Set generation priority per area
- Trigger page generation for specific area
...
- Association to content (which draft/page/blog it belongs to)
...
**Requirements:**
- Database-driven navigation (not hardcoded arrays in code)
- Admin UI to add/remove/reorder menu items
- Support for sub-menus (nested items)
- Separate management for: public site header, public site footer, admin sidebar
...
**Before starting ANY new phase, feature, or significant code change:**
```
Decision Record (Mandatory):
  1. What are we building?
  2. Why NOW (not later)?
  3. What is the impact on existing system?
  4. What are the dependencies?
  5. What is the rollback plan?
  6. CEO approval obtained? (date + method)
```
...
```
1. Large phases are broken into sub-phases (e.g., 3a, 3b, 3c)
2. Independent sub-phases CAN run in parallel (like mosaic tiles)
...
| Rule | Description |
|------|-------------|
| W1 | Worker reads bible FIRST before starting any work |
| W2 | Worker claims sub-phase explicitly (marks IN PROGRESS) |
| W3 | Worker does NOT touch files outside their sub-phase scope |
...
| 7 | System Contract | Every module has a written input/output contract. |
| 8 | Test Coverage | Happy flow + edge case + failure case + load test. |
| 9 | Observability | Every event logged with reason, level, and context. |
...
| 26 | CEO Supremacy | CEO's word = final. System never overrules CEO. Override capability on everything. |
| 27 | Communication Guarantee | Alert = delivered to CEO's device. DB-only alert = no alert. Multi-channel mandatory. |
| 28 | Unified Content | CEO sees ALL content (CCC + pages + blog + downloads) in one dashboard. |
| 29 | Geo Control | CEO adds city/locality/pincode from admin. No code touch required. |
| 30 | Media System | Supabase Storage, image ratios, alt text, content association. No local filesystem. |
| 31 | Navigation Management | Database-driven menus. Admin UI to edit. ONE navigation system, not two. |
| 32 | Decision Documentation | Every decision answers: Why now? Why not later? Impact? Documented before coding. |
| 33 | Worker Execution | Mosaic model: independent workers, no file conflicts, CTO joins and verifies. |

---

> **These rules are not a burden. They are the reason this system will be stable at 10,000 pages, at 10,000 leads/month, and at 3 years of continuous operation. Skip them once, pay the price for months.**
...
```
ACT AS: CHIEF TECHNOLOGY OFFICER (CTO) + SYSTEM ARCHITECT + FORENSIC AUDITOR + QUALITY GATEKEEPER

You are NOT a developer.
You are the SYSTEM OWNER.

You do NOT just build.
You ENSURE:
  - Nothing is missing
  - Nothing is fragile
...
Build a **PRODUCTION-GRADE, SELF-SCALING, ZERO-BREAKAGE SYSTEM** where every module is:

| Quality | Meaning |
|---------|---------|
| **Complete** | All sub-modules finished, no half-done features |
| **Tested** | All 4 test types passed (happy, edge, failure, load) |
...
5. Identify future scaling risks
...
|----------|-----------------|
| Missing features | Does the bible define something not yet built? |
| Hidden risks | What can break silently? |
| SEO gaps | Duplicate content? Missing metadata? Broken internal links? |
| Scaling issues | Will this work at 10,000 pages? |
| UX gaps | Can the admin actually use this? Is it confusing? |
...
- Proper slug structure (clean, lowercase, hyphenated)
- Internal linking exists between related pages
...
2. Add any missing sections discovered during implementation
3. Improve unclear definitions
4. Add newly discovered requirements
...
2. Provide a specific fix plan
3. Implement the fix after CEO approval
4. Re-score until all are 10/10
...
|--------------------------|----------------------|
| CTO follows instructions blindly | CTO THINKS, DETECTS, PREVENTS, PERFECTS |
| CEO micro-manages every detail | System self-corrects through protocol |
| Gaps found after deployment | Gaps caught before first line of code |
| "Should work" culture | "Proved working" culture |
| Modules half-done, moved to next | Modules 10/10 complete before next |
...
licnewdelhi.com ranks well for LIC-related queries with a simple strategy: it acts as a **repository of LIC forms + information** that people genuinely need. The insight is:
...
| No bulk scheduling | MEDIUM | Can only generate 1 page at a time |
| No internal linking | MEDIUM | Pages are isolated islands, zero SEO authority transfer |
| Downloads are free (no lead gate) | HIGH | Revenue opportunity left on table |
| No form guidance service | HIGH | Biggest conversion opportunity unmissed |
| Single lead funnel (Bima Sakhi only) | HIGH | Male recruit / policy buyer / IC-38 leads get wrong follow-up |
| No lead scoring | HIGH | Agent wastes time on cold leads, misses hot ones |
| Content only in English | MEDIUM | Hindi-first users disengage; tier 2/3 city penetration suffers |
| No page performance tracking | HIGH | No visibility into what's working, no self-improvement loop |
| GTM / GA4 / GSC not properly wired | MEDIUM | No attribution, no analytics, no conversion data |
| Contact page form not CRM-connected | MEDIUM | Direct intent leads lost, no follow-up triggered |
| No content variation rules | MEDIUM | Same-structure pages risk Google duplicate penalty at scale |
| No agent personalization on pages | MEDIUM | Generic pages convert worse than "meet your local agent" pages |
| Static thank-you page (no intent match) | MEDIUM | Post-conversion experience is cold, user drops off |
...
This is the most important issue in the entire codebase. It must be fixed before anything else.

**The pagegen worker writes to:**
- `page_index` table (slug, status, city_id, etc.)
- `location_content` table (hero_headline, body content, FAQs, meta)

**The frontend page router (`app/pages/[slug]/page.js`) reads from:**
- `custom_pages` table (a completely different table used by the CMS block builder)

**Result:** Every page generated by pagegen is stored in the database but **has no URL that serves it**. If you visit `bimasakhi.com/lic-agent-in-delhi`, you get a 404. The sitemap might list the URL, but nothing exists at that URL. Google will crawl, get 404, and penalize.

### Proof of the Gap

```
Pagegen writes:
  page_index.page_slug = "lic-agent-in-krishna-nagar-delhi"
...
## 5. Engine 1: Geo + Intent Intelligence

### Geo Hierarchy (Already in DB, Need to Use Better)
...
| Scope | Slug Pattern | Target Query | Competition |
|-------|-------------|-------------|-------------|
| `national` | `/lic-agent-india` | "best LIC agent India" | Very High |
| `state` | `/lic-agent-delhi-ncr` | "LIC agent Delhi NCR" | High |
| `city` | `/lic-agent/delhi` | "LIC agent in Delhi" | Medium |
| `locality` | `/lic-agent/delhi/krishna-nagar` | "LIC agent in Krishna Nagar" | Low |
| `pincode` | `/lic-agent/110051` | "LIC agent near 110051" | Very Low |
| `micro` | `/lic-agent/delhi/krishna-nagar/lal-quarter` | "LIC agent Lal Quarter" | Almost Zero |
...
|--------|--------------|--------------|----------|
| `local_service` | Bima Sakhi agent in area | "LIC agent in Dwarka" | Recruitment |
| `policy_info` | Specific LIC policy details | "LIC Jeevan Anand kya hai" | Policy buyer |
| `career` | Become LIC agent journey | "LIC agent kaise bane Patna" | Recruitment |
| `educational` | Financial literacy guide | "life insurance ki zaroorat kyun hai" | Top of funnel |
| `transactional` | Apply now, enroll now | "apply LIC Bima Sakhi Delhi" | Hot lead |
| `form_help` | IC-38 form / registration help | "LIC agent form kaise bhare" | Form service lead |
| `download` | Study material, form download | "LIC IC-38 form download" | Lead magnet |
| `comparison` | Term vs endowment | "term life vs endowment LIC" | Policy buyer |

### Master Page Context Object (What AI Gets Per Page)
...
```json
{
  ...
  "pincode": "110051",
  "sub_areas": ["Lal Quarter", "Azad Nagar", "Govind Pura", "Ram Nagar"],
  "nearby_landmarks": ["Krishna Nagar Metro Station", "Laxmi Nagar Market"],
  "target_keyword": "LIC agent in Krishna Nagar Delhi",
  "secondary_keywords": [
    "LIC advisor near Krishna Nagar",
    "life insurance agent 110051",
    "Bima Sakhi Krishna Nagar"
  ],
...
## 6. Engine 2: Content Command Center Dashboard
...
Cards to show:
- **Drafts:** Total | Pending Review | Approved | Rejected | Scheduled
- **Publishing:** Pages live today | This week | Total active
- **Quality:** Avg word count | Avg quality score | Review queue backlog
- **Traffic (future):** Organic impressions | Clicks | Top ranking page
- **Leads from SEO:** Count today | This week | Attribution by page

### Draft Review Queue (`/admin/ccc/drafts`)

**Filters:**
- Status: All | Draft | Review | Approved | Scheduled | Published | Rejected
...
**Bulk actions:** Approve selected | Reject selected | Schedule selected | Delete selected

### Full Draft Editor (`/admin/ccc/drafts/[id]`)
...
## 7. Engine 3: Page Templates + Layouts + Images

### Page Template System

Generated pages need a proper visual template. The existing `custom_pages` CMS uses block-based rendering. Generated pages (from `location_content`) need their own dedicated template.

### Template: `GeneratedPageTemplate` Component

**File:** `components/layout/GeneratedPageTemplate.jsx`
...
Different `intent_type` values use different layout variants:

| Intent Type | Layout Variant | Key Sections |
|-------------|---------------|-------------|
| `local_service` | Service (default above) | Hero, Content, Apply Form, FAQ, Nearby Areas |
| `career` | Career | Hero, Journey Steps, Income Calculator widget, Apply Form, FAQ |
| `policy_info` | Policy | Hero, Policy Details, Premium Calculator link, FAQ, CTA |
| `educational` | Blog | Hero (no form), Content, CTA at bottom, Related posts |
| `form_help` | Tool | Instructions, Document checklist, Download CTA (gated), FAQ |
| `download` | Download | Resource list, Lead gate form, What you get, FAQ |

### Schema Markup (Auto-Generated)

Every generated page automatically gets:
...
```json
{
  ...
  "mainEntity": [
    {
      "@type": "Question",
      "name": "LIC agent kaise bane Krishna Nagar mein?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Krishna Nagar mein LIC agent banne ke liye..."
      }
    }
  ]
}
...
**LocalBusiness Schema** (for local_service pages):
```json
{
  "@context": "https://schema.org",
  "@type": "LocalBusiness",
  ...
  "areaServed": "Krishna Nagar, Delhi"
}
```

**BreadcrumbList Schema** (auto from geo hierarchy):
```json
{
  "@context": "https://schema.org",
  "@type": "BreadcrumbList",
  "itemListElement": [
    ...
    {"@type": "ListItem", "position": 2, "name": "LIC Agent Delhi", "item": "https://bimasakhi.com/lic-agent/delhi"},
    {"@type": "ListItem", "position": 3, "name": "Krishna Nagar", "item": "https://bimasakhi.com/lic-agent/delhi/krishna-nagar"}
  ]
}
```
...
The AI generates platform-optimized image prompts for each page. These are stored in `content_drafts.image_prompts` JSONB and displayed in the draft editor.
...
Bottom: 'bimasakhi.com' in small white text. 
Right side: small icon of woman in professional attire. No photos."
```
...
Background: blurred warm interior. Real photography style. Horizontal."

OG IMAGE:
"Social media card with bold text: 'Become a LIC Agent in [CITY]' 
Subtitle: 'Financial Independence. Flexible Hours. Real Income.'
Illustrated icons: rupee coin, calendar, shield. 
Color: white background, emerald + navy accent. Professional clean layout."
```
...
The editor shows 3 variants of each prompt:

| Platform | Optimizations |
|----------|--------------|
| **Canva (Magic Media)** | Add "Canva style", specify art style keywords Canva responds to, format for "Generate image" prompt box |
| **Adobe Firefly** | "Firefly-generated", content type tags, style references, avoid copyright triggers |
| **Google Imagen/Flow** | Google's preferred descriptors, photorealistic tags, safety-compliant phrasing |

---

## 8. Engine 4: Publish + Distribution + Sitemap

### Auto-Sitemap on Publish

When `content_drafts.status` changes to `published`:

1. Set `page_index.status = 'active'`
2. Sitemap auto-includes it on next crawl (already works via existing sitemap routes)
3. Ping Google Search Console (future: via GSC API)

### Enhanced Sitemap Architecture

Current sitemap has 3 types. Extend to:
...
```json
{
  "pillar": { "slug": "lic-agent-delhi", "text": "LIC Agent in Delhi", "relationship": "parent_city" },
  "cluster": [
    { "slug": "lic-agent-delhi-laxmi-nagar", "text": "LIC Agent in Laxmi Nagar", "relationship": "sibling_locality" },
    { "slug": "lic-agent-delhi-shahdara", "text": "LIC Agent in Shahdara", "relationship": "sibling_locality" }
  ],
  "cross": [
    { "slug": "become-lic-agent-delhi", "text": "Become a LIC Agent in Delhi", "relationship": "career_cta" },
    { "slug": "lic-jeevan-anand-policy", "text": "LIC Jeevan Anand Policy Details", "relationship": "related_product" }
  ]
}
```
...
## 9. Download-as-Lead-Magnet System

### The Strategy (Inspired by licnewdelhi.com, Done Better)

licnewdelhi.com ranks by being a simple form repository. We do the same but:
...
| Resource | Search Intent | Lead Quality |
|----------|--------------|-------------|
| IC-38 Exam Form | "IC-38 registration form" | Very High |
| IC-38 Syllabus PDF | "IC-38 syllabus download" | High |
| IC-38 Model Question Papers | "IC-38 mock test" | High |
| LIC Agent Document Checklist | "LIC agent documents required" | Very High |
| Policy Comparison Chart | "LIC term vs endowment comparison" | Medium |
| Income Calculator Sheet | "LIC agent income calculation" | Medium |
| Customer Proposal Form | "LIC policy proposal form" | Very High |
| IRDAI Agent Code Rules | "LIC agent rules regulations" | Medium |
| City-Specific LIC Office List | "LIC office [city] address" | High |

### Downloads Admin (`/admin/ccc/downloads`)

Manage all downloadable resources:
- Upload new PDF/resource
- Set lead gate: ON/OFF (some resources can be free, some gated)
- View lead count per resource
- Download conversion rate
- Edit associated landing page content

---

## 10. Form Guidance + Agent Customization Service
...
- Visual field-by-field guide (annotations on form image)
- Document checklist with examples
- Common mistakes to avoid
- "If you need help, our agent will guide you FREE" CTA
...
## 11. Zoho One Integration

### Available Zoho Apps (Relevant Ones for CCC)

Zoho One includes 45+ apps. These 8 are directly relevant to the CCC vision:
...
#### 2. Zoho SalesIQ (Live Chat + Chat Bot)
**Use:** Convert website visitors in real-time

**Integration:**
- Embed on all generated pages
- Bot triggered when user visits `/forms/` or `/downloads/` pages
- Bot message: "Kya aapko IC-38 form mein help chahiye? Main guide kar sakta hun!"
- Captures name + mobile before agent takes over
- Full transcript saved in Zoho CRM

#### 3. Zoho Campaigns (Email Marketing)
...
**Integration:**
When a page is approved + published in CCC:
- Auto-create social post draft (using page meta_title + featured_image)
- Optional: auto-publish to Facebook Page, Instagram, LinkedIn Company Page
- Zoho Social handles scheduling

#### 5. Zoho Cliq (Internal Team Messaging)
...
#### 7. Zoho Bookings
**Use:** Let leads book a call directly from the website

**Integration:**
- After download gate: "Book a FREE 15-min call with our agent"
- Booking form embeds on `/forms/` pages
- Auto-creates Zoho CRM appointment
- Reminder sent via WhatsApp + Email

#### 8. Zoho Sign
**Use:** Digital signatures for agent enrollment forms
...
### Current State
- GBP exists and drives traffic (primarily LIC service related queries)
- But GBP is underutilized as a content + lead channel

### GBP + CCC Integration

**1. GBP Posts (Weekly Content)**

Every time a new city/locality page is published:
...
Seed 10-15 Q&As that match high-intent searches:
- "How to become LIC agent in Delhi?"
- "LIC agent near Laxmi Nagar?"
- "Bima Sakhi kya hai?"

These Q&As rank directly in Google Maps results.

**3. GBP Products Section**

Add LIC products as "services":
- Bima Sakhi Agent Registration
- IC-38 Exam Guidance
...
**5. GBP + Local Page Consistency**

For every city page published on the website:
- Ensure NAP (Name, Address, Phone) consistency
- Add city/locality as a service area in GBP
- GBP service area pages reinforce the website's local relevance signals

---

## 13. Multi-City + Pincode Micro-Local Engine
...
A single pincode covers multiple delivery offices = multiple micro-areas.

Example for pincode `110051` (Krishna Nagar, Delhi):
```
areas:
...
Data source: India Post's open pincode data (free, public domain)

**Enhanced import script:** Extend `scripts/import-pincodes.js` to also import sub-areas from India Post data or OpenCity API.
...
## 14. Anti-Spam + Google Safety System

Google penalizes "programmatic content" that is:
- Thin (< 600 words)
- Duplicative (same content, different city name swapped)
- Doorway pages (no genuine local value)
- Keyword stuffed (> 3% density)
- Orphan pages (no internal links)

### Our 6-Layer Protection System

#### Layer 1: Content Quality Gates (In Pagegen Worker)
...
System-level daily limits (enforced in `content_drafts` publish logic):

```
daily_publish_limits:
  micro_pages: 30/day
  locality_pages: 20/day
  city_pages: 5/day
  state_pages: 2/day
  national_pages: 1/day
  TOTAL CAP: 50/day regardless of mix
```

Why? Google's crawl budget is finite. Slow, quality drip beats burst publishing.

#### Layer 4: Mandatory Internal Linking

A page CANNOT be published without:
...
By default, ALL generated drafts require human approval before publishing. This can be relaxed for high-scoring (> 8.0 quality score) pages once the system is trusted.
...
Even after approval, sitemap additions are rate-limited:
- Max 50 new URLs added to sitemap per day
- Priority: city > locality > micro
- `sitemap-keywords-latest.xml` shows only last 200 active pages (fresh signal)

---

## 15. New Database Schema

### New Migration: `035_content_command_center.sql`

```sql
-- ============================================================
-- Content Drafts (The Human Control Layer)
-- ============================================================
CREATE TABLE IF NOT EXISTS content_drafts (
    ...
    created_by TEXT DEFAULT 'pagegen', -- 'pagegen' | 'admin_manual' | 'bulk_job'
    bulk_job_id UUID REFERENCES bulk_generation_jobs(id),
    
    -- Linked to page_index (created when draft is created, status='draft' until approved)
    ...
    pincode TEXT,
    scope TEXT DEFAULT 'locality' 
        CHECK (scope IN ('national','state','city','locality','pincode','micro')),
    intent_type TEXT DEFAULT 'local_service'
        CHECK (intent_type IN ('local_service','policy_info','career','educational',
                               'transactional','form_help','download','comparison')),
    ...
    hero_headline TEXT,
    body_content TEXT,              -- Full HTML content
    faq_data JSONB DEFAULT '[]',    -- [{q: "...", a: "..."}]
    cta_text TEXT,
    
    -- Taxonomy
    tags TEXT[] DEFAULT '{}',       -- ['retirement', 'women', 'delhi']
    category TEXT,                  -- 'service' | 'policy' | 'blog' | 'career' | 'form'
    content_type TEXT,              -- 'city_page' | 'locality_page' | 'blog_post' etc
    ...
    internal_links JSONB DEFAULT '{}',  -- {pillar: {...}, cluster: [...], cross: [...]}
    
    -- Image Intelligence
    image_prompts JSONB DEFAULT '{}',   -- {hero: {canva:"...", adobe:"...", imagen:"..."}, thumbnail: {...}, og: {...}}
    ...
    status TEXT DEFAULT 'draft' 
        CHECK (status IN ('draft','review','approved','scheduled','published','rejected','archived')),
    review_notes TEXT,
    word_count INTEGER DEFAULT 0,
    quality_score NUMERIC(3,1),
    uniqueness_score NUMERIC(3,1),
    local_context_score NUMERIC(3,1),
...
CREATE INDEX idx_content_drafts_intent ON content_drafts(intent_type);
CREATE INDEX idx_content_drafts_city ON content_drafts(city_id);
CREATE INDEX idx_content_drafts_slug ON content_drafts(slug);
CREATE INDEX idx_content_drafts_scheduled ON content_drafts(scheduled_publish_at) 
...
CREATE TABLE IF NOT EXISTS bulk_generation_jobs (
    ...
    auto_approve_threshold NUMERIC(3,1) DEFAULT 8.0,  -- Auto-approve if quality >= this
    require_review_below NUMERIC(3,1) DEFAULT 6.0,    -- Always flag for review if quality < this
...
-- ============================================================
-- Pincode Micro-Areas (Sub-localities within a pincode)
-- ============================================================
CREATE TABLE IF NOT EXISTS pincode_areas (
    ...
    area_type TEXT CHECK (area_type IN ('block','sector','colony','market','landmark','village','extension')),
    population_estimate INTEGER,
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(pincode, area_slug)
);

CREATE INDEX idx_pincode_areas_pincode ON pincode_areas(pincode);
CREATE INDEX idx_pincode_areas_locality ON pincode_areas(locality_id);


-- ============================================================
-- Keyword Clusters (from Ahrefs / Ubersuggest CSV import)
-- ============================================================
...
CREATE INDEX idx_keyword_clusters_intent ON keyword_clusters(intent_type);
CREATE INDEX idx_keyword_clusters_priority ON keyword_clusters(priority_score DESC);


-- ============================================================
-- Download Resources (Lead Magnet Manager)
-- ============================================================
CREATE TABLE IF NOT EXISTS download_resources (
    ...
    slug TEXT UNIQUE NOT NULL,
    description TEXT,
    resource_type TEXT CHECK (resource_type IN ('pdf','excel','checklist','guide','template','form')),
    file_url TEXT NOT NULL,          -- Storage URL (Supabase Storage or external)
    thumbnail_url TEXT,
    
    -- Lead Gate
    is_gated BOOLEAN DEFAULT TRUE,   -- If true, requires lead form before download
    gate_fields JSONB DEFAULT '["name","mobile","email","city"]',
    lead_tag TEXT,                   -- Tag added to CRM lead (e.g., 'ic38_form_download')
    
    -- Lead Stats
    view_count INTEGER DEFAULT 0,
    download_count INTEGER DEFAULT 0,
    lead_count INTEGER DEFAULT 0,
    conversion_rate NUMERIC(4,1) DEFAULT 0,
...
-- ============================================================
-- Content Publish Log (Audit trail for all publish actions)
-- ============================================================
CREATE TABLE IF NOT EXISTS content_publish_log (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    draft_id UUID REFERENCES content_drafts(id),
    action TEXT CHECK (action IN ('approved','rejected','published','scheduled','unpublished','archived')),
    performed_by TEXT,
    notes TEXT,
...
-- Add content_type to page_index for sitemap routing
ALTER TABLE page_index ADD COLUMN IF NOT EXISTS content_type TEXT DEFAULT 'locality_page';
ALTER TABLE page_index ADD COLUMN IF NOT EXISTS intent_type TEXT DEFAULT 'local_service';
ALTER TABLE page_index ADD COLUMN IF NOT EXISTS scope TEXT DEFAULT 'locality';
```
...
2. Modify pagegen worker (`app/api/jobs/pagegen/route.js`):
   - After AI generation, write to `content_drafts` table (status='draft')
   - Keep writing to `page_index` + `location_content` as before (no breakage)
   - But set `page_index.status = 'pending_index'` (NOT 'active') until human approval
3. Create `/admin/ccc/` overview page (basic stats: draft count, pending review, published)
4. Create `/admin/ccc/drafts` list page (table with filter by status/city/quality)
5. Create `/admin/ccc/drafts/[id]` editor (left/center/right panel layout)
   - All metadata editable
   - Content editable (rich text or textarea)
   - Approve button: sets draft.status='approved', page_index.status='active'
...
1. Extend pagegen worker: generate image prompts alongside content
   - Add `generateImagePrompts(pageContext, aiContent)` function to `lib/ai/generateContent.js`
   - Store in `content_drafts.image_prompts` JSONB
2. Display in draft editor right panel:
...
2. Create `/admin/ccc/bulk` planner page
   - Select: cities (multi-select), intent type, scope, base keyword, daily limit
   - Preview: page count estimate, cost estimate, completion time
   - Create job button
3. Bulk job runner: `app/api/admin/ccc/bulk/[id]/start/route.js`
   - Creates N `generation_queue` entries with rate limiting
   - Sets `bulk_job_id` on each draft
4. Bulk job progress monitor in UI
...
2. Create `/admin/ccc/downloads` manager
3. Upgrade `/app/downloads/DownloadsContent.jsx`:
   - Implement lead gate modal (shows before download)
   - On form submit: POST to `/api/leads` with `source='download'` + `intent=resource.lead_tag`
   - On success: reveal download link (pre-signed URL or direct link)
   - Track `download_resources.download_count` + `lead_count`
4. Auto-trigger Zoho CRM + WhatsApp notification on download lead
...
2. Create `/admin/ccc/keywords` page with CSV upload
...
2. Zoho CRM: enhance lead sync with `intent_type`, `source_page_slug`, `download_resource` tags
3. Zoho Campaigns: set up email sequences by intent type
4. Zoho SalesIQ: embed on form help + download pages
5. GBP post automation: on page publish, create GBP post draft
...
|--------|--------|----------------|
| Pages published (live) | 300+ | `page_index` count where status='active' |
| Google indexed | 200+ | Google Search Console |
| Organic impressions/month | 10,000+ | GSC |
| Organic clicks/month | 500+ | GSC |
| Download leads/month | 100+ | `crm_leads` where source='download' |
| Organic page leads/month | 50+ | `crm_leads` where source_page_slug IS NOT NULL |
| Agent enrollments from SEO | 5+ | CRM pipeline tracking |
| Pages ranking top 10 | 30+ locality pages | GSC rank data |
...
## 18. The Competitive Moat

### Why This Is Hard to Copy

1. **Geo depth:** Most competitors do city-level. You'll do pincode + micro-area level. Building 2,000 locality pages takes months and requires the infra you're already building.
...
> Ek mediocre page ka koi fayda nahi. 50 brilliant local pages se 50 qualified leads aate hain jo agents ki life change kar dete hain. Quality is the non-negotiable foundation. Scale is built on top of quality, never instead of it.
...
Right now the entire lead capture flow assumes ONE identity: a woman interested in becoming a Bima Sakhi agent. But as content grows to cover:
- Male LIC agent recruitment
- Policy-specific content (Jeevan Shanti, Jeevan Anand, Term Plans)
- Form help pages (IC-38 guidance)
- Download resources
- Educational blog readers
- Comparison researchers

...each of these visitors has a completely different intent, a completely different next step, and needs a completely different funnel.
...
| `educational` | Curious, early awareness | Read, build trust, come back | AWARENESS | `funnel=awareness` |

### Each Funnel Has Its Own:

**1. CTA Text on Page**
- Bima Sakhi: "Bima Sakhi ban kar apni pehchaan banao"
- Male Agent: "LIC ka authorized agent ban kar kamayi shuru karo"
- Policy Buyer: "Aaj hi apni policy ke baare mein expert se baat karo"
- Annuity: "Retirement planning ke liye FREE consultation book karo"

**2. Lead Capture Form Fields**
- Recruit funnel: Name, Mobile, City, Current Occupation, Education (min 10th pass required)
- Policy buyer: Name, Mobile, City, Age, "Kya aap insurance lena chahte hain?" (Yes / Explore)
- IC-38 help: Name, Mobile, City, "IC-38 exam kitne din mein dena chahte ho?"
- Form help: Name, Mobile, City, "Kaunsa form hai jo aapko help chahiye?"
...
- Recruit: "Namaste! Bima Sakhi program ke baare mein aapki inquiry mili. Aaj 5 minute hai baat karne ke liye?"
- Policy: "Namaste! Aapki insurance inquiry mili. Main [City] ke liye LIC expert hu. Kya policy discuss kar sakte hain?"
- IC-38: "Namaste! IC-38 exam preparation mein main personally guide kar sakta hun. Kab free hain?"
- Download: "Aapka [resource name] download ho gaya! Koi sawaal ho to hum yahan hain."

### How the System Detects Intent

The page generates the lead form. Every page knows its `intent_type` (stored in `content_drafts.intent_type`). This is passed to the lead form as a hidden field:

```
form hidden fields:
  source_page_slug: "lic-jeevan-shanti-plan-delhi"
  intent_type: "annuity_buyer"
  content_type: "policy_info"
  city: "Delhi"
  funnel: "annuity_sale"
```

When the form is submitted via `/api/leads`:
1. Lead is created in `crm_leads` with all intent tags
2. Zoho CRM contact is created/updated with matching tags
3. Correct email sequence is triggered in Zoho Campaigns
...
```
Lead Source Tags (ALL must be set):
  source_channel: "organic_seo" | "download" | "direct" | "referral" | "social"
  funnel: "sakhi_recruit" | "agent_recruit" | "policy_sale" | "annuity_sale" | 
           "term_sale" | "ic38_guide" | "form_service" | "nurture_download" | "awareness"
  intent_type: (raw intent from page: "bima_sakhi_recruit" | "male_agent_recruit" | etc)
  source_page_slug: "lic-agent-in-krishna-nagar-delhi" (the exact page that generated the lead)
  city: "Delhi"
  locality: "Krishna Nagar"
...
- No recruit lead ever gets a policy sale email sequence
- No policy lead is mistakenly contacted about becoming an agent
- Analytics can cleanly separate SEO ROI by funnel type

### New DB Columns Required

```sql
-- On crm_leads table:
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS funnel TEXT;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS intent_type TEXT;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS source_page_slug TEXT;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS locality TEXT;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS download_resource TEXT;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS lead_score INTEGER DEFAULT 0;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS lead_priority TEXT 
...
### Two Dangerous Failure Modes to Avoid

**Failure Mode A: Blind Automation**
System runs, does things, user has no visibility. A bad AI batch publishes 500 thin pages. Google penalizes. You don't know until traffic drops 80%.

**Failure Mode B: Paralysis by Confirmation**
Every single action requires your approval. Nothing scales. You spend 6 hours/day clicking approve buttons.
...
- **Alter:** Change the targeting, daily limit, keyword, or AI prompt before a job runs
- **Pause:** Stop a running job mid-way. It resumes from where it stopped.
- **Scrap:** Cancel + archive everything. No pages published, drafts marked as cancelled.
- **Re-run:** If you scrapped something, start fresh with new parameters.
...
**Every automated action is logged** in `content_publish_log` with:
- What happened
- What triggered it
...
### Component 1: Page Performance Brain

**Data per page tracked:**

| Signal | Source | Update Frequency |
|--------|--------|-----------------|
| Impressions | Google Search Console API | Weekly |
| Clicks | Google Search Console API | Weekly |
| Average position | Google Search Console API | Weekly |
| CTR | Computed from above | Weekly |
| Leads generated | `crm_leads.source_page_slug` | Real-time |
| Lead-to-click rate | Computed | Weekly |
| Bounce rate | GA4 API | Weekly |
| Time on page | GA4 API | Weekly |

Stored in new table `page_performance_snapshots` (weekly snapshot, not real-time):
...
CREATE TABLE page_performance_snapshots (
    ...
    lead_to_click_rate NUMERIC(6,3),
    UNIQUE(page_slug, snapshot_date)
);
```

### Component 2: AI Pattern Analyzer

Every 2 weeks, a scheduled job runs:
...
"These are our top performing pages: [list with their content structure, word count, FAQ count, 
 intent type, city, locality, heading structure, internal links count].
 These are our underperforming pages: [same data].
 
 Analyze the patterns. What makes a page perform well?
 Give me:
 1. Top 5 content patterns that correlate with leads
 2. Top 3 structural patterns that correlate with ranking
 3. 3 specific recommendations to improve underperforming pages
 4. Recommended changes to the AI prompt template for future generation"

Output stored in: admin_insights table (with CEO review required before any action)
```

CEO sees the AI's analysis in `/admin/ccc/insights` and decides:
- Apply recommendation to underperforming pages (triggers re-optimization)
- Update AI prompt template for future generations
- Scale the winning content pattern to more cities/localities
...
**Current plan (basic):**
- Parent always gets a link from child
- Sibling pages link to each other
...
**Admin: Link Map Visualization** `/admin/ccc/sitemap`
Visual graph showing which pages link to which. Orphan pages (no inbound links) highlighted in red. Dense link clusters shown in green.
...
All leads look the same in CRM. But:
- A woman who visited the Bima Sakhi page 3 times, downloaded the IC-38 syllabus, and then filled the recruitment form = EXTREMELY HOT LEAD
- Someone who accidentally submitted the contact form while trying to find an office address = COLD LEAD

If the agent treats both the same, they waste time and burn out.

**Lead Scoring ensures agent time goes to the highest-probability conversions.**

### Scoring Factors

**Behavioural Signals (tracked via GA4 / session cookie):**

| Signal | Points | Reasoning |
|--------|--------|-----------|
| Visited recruitment/service page | +15 | Showed intent to the topic |
| Visited 3+ pages on same visit | +10 | High engagement |
| Returned for a 2nd visit | +20 | Voluntary re-engagement = strong interest |
| Downloaded a resource | +30 | Very high intent, active preparation |
| Visited form help page | +20 | Moving toward action |
| Submitted a lead form | +50 | Base score for any lead submission |
| Spent > 3 min on a page | +10 | Deep reading = genuine interest |

**Demographic / Profile Signals (from form submission):**
...
- Language toggle exists (`LanguageContext.jsx`) with Hindi and English
- Current UI switches display language
- But: pagegen generates content in one language only (English by default)
- No bilingual content stored anywhere in the DB

### The Bilingual Generation Plan
...
```sql
-- Add to content_drafts table:
ALTER TABLE content_drafts ADD COLUMN IF NOT EXISTS hero_headline_hi TEXT;
ALTER TABLE content_drafts ADD COLUMN IF NOT EXISTS body_content_hi TEXT;
ALTER TABLE content_drafts ADD COLUMN IF NOT EXISTS faq_data_hi JSONB DEFAULT '[]';
ALTER TABLE content_drafts ADD COLUMN IF NOT EXISTS cta_text_hi TEXT;
ALTER TABLE content_drafts ADD COLUMN IF NOT EXISTS meta_title_hi TEXT;
ALTER TABLE content_drafts ADD COLUMN IF NOT EXISTS meta_description_hi TEXT;

-- Similar for location_content table
ALTER TABLE location_content ADD COLUMN IF NOT EXISTS hero_headline_hi TEXT;
ALTER TABLE location_content ADD COLUMN IF NOT EXISTS faq_data_hi JSONB DEFAULT '[]';
ALTER TABLE location_content ADD COLUMN IF NOT EXISTS body_content_hi TEXT;
```

### Language-Aware Page Rendering

The `GeneratedPageTemplate` component reads user's selected language from `LanguageContext`:

```jsx
const { language } = useLanguage(); // 'en' | 'hi'

const headline = language === 'hi' ? data.hero_headline_hi : data.hero_headline;
const faqs = language === 'hi' ? data.faq_data_hi : data.faq_data;
const body = language === 'hi' ? data.body_content_hi : data.body_content;
```

**SEO Consideration:** The canonical URL stays the same for both languages. The `lang` attribute on `<html>` and `hreflang` tags are set based on the active language context. Primary indexing is English, but Hindi content improves engagement metrics (time on page, bounce rate) which also signals quality to Google.

### Future: Area-Specific Language Support

India has 22 official languages. The vision:

```
Phase 1 (NOW):    Hindi + English (all pages)
Phase 2 (Later):  Add Marathi for Maharashtra cities
                  Add Bengali for Kolkata + West Bengal
                  Add Tamil for Chennai + Tamil Nadu
                  Add Telugu for Hyderabad + Andhra/Telangana
...
**AI capability:** Gemini 2.0 Flash supports all major Indian languages natively. The prompt template simply changes the language instruction.

### Draft Editor: Language Tab View
...
If locality pages for Krishna Nagar and Laxmi Nagar have:
- Same structure
- Same opening paragraph (with only city name swapped)
- Same FAQ questions
- Same CTA
...
- Local problem statement: "Delhi mein mehengaai ke is daur mein..."
- Question hook: "Kya aap jaante hain ki aapke ghar ke paas ek mahila LIC agent aapka wait kar rahi hai?"
- Stat hook: "Delhi mein 1 lakh se zyada families ne LIC se apna future secure kiya hai..."

**Axis 3: FAQ Structure**
- Some pages: 5 short FAQ pairs
- Some pages: 3 detailed FAQ pairs with extended answers
- Some pages: Mix of factual + emotional FAQs
- Every page: At least 1 FAQ must be city+locality specific (not reusable)

**Axis 4: Local Context Injection**
- Reference nearby market or landmark (from geo data in pincode_areas)
- Mention typical occupation of residents (from city context data)
- Reference local transport or commute pattern when relevant

**Axis 5: CTA Style**
- Urgency: "Sirf 5 seats baaki hain is mahine ke orientation mein"
- Benefit: "Free training + immediate earning + flexible schedule"
- Social proof: "127 mahilaon ne is saal Krishna Nagar se apni journey shuru ki"
- Personal: "Main aapka pehla step free mein guide karunga"

**Axis 6: Testimonial Style**
- Formal: "Meena Ji, 38, Krishna Nagar resident..."
- Casual: "Pooja apne ghar se hi kaam karti hai aur mahine mein 25,000 kamaati hai..."
- Dialogue: " 'Mujhe nahi pata tha ki main ye kar sakti hun,' kehti hain Sunita..."
...
{
  "tone": "storytelling",
  "hook_type": "local_landmark",
  "faq_style": "mixed",
  "cta_style": "social_proof",
  "testimonial_style": "casual"
}
```

The AI uses this as strict instructions, not suggestions. The quality reviewer checks that the variation was actually applied (not just the city name swapped).
...
Every page you publish is a content asset. Most teams stop at the page. We extract 5+ pieces of content from every single page:
...
Example hashtags: #LICIndia #FinancialIndependence #BimaShakhi #WomenEmpowerment #DelhinCR
```
...
```
Slide 1: Hook headline (large text) + background image prompt
Slide 2: Problem statement ("Kya aap bhi...")
Slide 3: Solution ("Bima Sakhi program...")  
Slide 4: Key benefits (3 bullet icons)
Slide 5: How it works (3 steps)
Slide 6: CTA ("DM karo 'START' ya bio link pe jao")

Note: Actual carousel is created manually in Canva using these slides as content guide.
      AI generates the text + image prompt per slide, human creates the visual.
```
...
CTA: "Description mein link hai" or "Follow karo aur save karo"
```
...
**CEO always has final control.** Social content is never auto-posted. Only auto-drafted.
...
**Data Structure: agent_areas table**
...
CREATE TABLE agent_areas (
    ...
    agent_designation TEXT DEFAULT 'LIC Development Officer',
    agent_tagline TEXT,   -- "Aapki financial independence ki partner"
    city_id UUID REFERENCES cities(id),
    locality_id UUID REFERENCES localities(id),
    pincode TEXT,
    ...
    calendly_link TEXT,   -- Direct booking link
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
```
...
Four tools, each playing a specific role. They must work together, not in isolation.
...
- GA4 event tags (form submissions, button clicks, downloads, language toggle)
...
**Next.js integration:** GTM script tag in `app/layout.js` using Next.js `Script` component with `strategy="afterInteractive"`.
...
- Impressions per page slug
- Clicks per page slug
- Average position per page slug
- CTR per page slug
...
**How we use it:**
1. After each new page template is deployed, run GT Matrix test on a sample page
2. Target scores: LCP < 2.5s, CLS < 0.1, TBT < 300ms (Grade A / B)
3. Common optimizations for Next.js:
...
**Periodic testing:**
- Test every 2 weeks on top 10 pages
...
Current state: One generic lead form. Everyone who submits goes to `/thank-you`. That's it.
...
**Forms are not generic data collectors. They are the beginning of a relationship.**

The form knows:
- What page the user was on (intent_type, funnel)
- What language they're using (Hindi / English)
- What city they've selected
- Whether they came from a download gate or an organic page

This data should make the form experience + post-form experience completely different.

### Dynamic Form Fields by Intent

**Bima Sakhi Recruit Form:**
```
...
Route: `/thank-you/[funnel-slug]`

**Each thank-you page has 3 zones:**
...
**Zone 2: Next Best Action (keeps user engaged)**
- Recruit: Show 2-3 success stories of existing Bima Sakhis + "Inse milein" CTA
- Policy: Show a comparison chart of policies (relevant to what they chose)
- IC-38: Show IC-38 exam schedule / dates (immediate value)
- Download: Show 2 related resources they might also want

**Zone 3: Social Proof / Trust Layer**
- Real lead count (e.g., "Is hafte 43 aur logon ne yehi decision liya")
- A GBP review screenshot (relevant to city if available)
- Agent photo + name for their city

### Smart Behavior: Content Adapts to User Intent

The `GeneratedPageTemplate` does not show a static sidebar form. It shows the intent-matched form:

```jsx
function SmartLeadForm({ intentType, funnel, city, pageSlug }) {
  // Returns the correct form variant based on intentType
  switch(intentType) {
    case 'bima_sakhi_recruit': return <BimasSakhiRecruitForm ... />;
    case 'male_agent_recruit': return <AgentRecruitForm ... />;
    case 'policy_buyer': return <PolicyConsultForm ... />;
    case 'annuity_buyer': return <AnnuityConsultForm ... />;
    case 'ic38_guide': return <IC38GuideForm ... />;
    case 'form_help': return <FormHelpForm ... />;
    default: return <GenericLeadForm ... />;
  }
}
```

### Post-Conversion Behavioral Flow (Future Phase)

After a lead submits:
...
The `/contact` page has a form that is either:
- Not properly connected to the CRM lead system
- Not tagged with correct intent / funnel
...
The contact page is NOT a generic "reach us" form. It should be a **lead qualification router**.
...
1. `/contact` page: Replace static form with dynamic intent-selector + conditional fields
2. Form submit: POST to `/api/leads` with `source_channel='direct'`, `funnel` derived from selected intent
3. Thank-you redirect: Based on selected intent, redirect to appropriate `/thank-you/[funnel]`
4. CRM: Lead correctly tagged and routed to right pipeline

**Lead scoring for contact form:**
- Intent selection = +20 points (they told us what they want)
...
AI Analysis job (bi-weekly):
  ...
  Output: admin_insights record with:
    - winning_patterns (content structure, word count, intent type, city tier)
    - recommended_prompt_changes
    - recommended_cities_to_expand
...
AI keyword discovery (monthly):
  Input: GSC "impressions but not clicked" queries (Search terms people are finding us for
         but we have no strong matching page)
  Output: "These 15 queries are bringing impressions but you have no dedicated page for them.
           Recommended: Create targeted pages for these queries."
...
Month 9:    1,000+ pages live. 100+ ranking in top 10.
            ...
            80% of lead form conversions are from organic (zero ad spend).
            Agents have more hot leads than they can handle.

Month 12:  Bimasakhi.com = THE authority for LIC services in India.
           No competitor has the geo depth, lead infrastructure, or intelligence layer
           that you have built over 12 months of compounding effort.
           
           This is no longer a website. It is a lead generation machine that
           runs on intelligence, not on ad budget.
```

### The CEO's Role in the Self-Growing System

You are not removed from the loop. You are elevated above it.
...
NEW ROLE (with this system):
  ...
  Everything else: automated, logged, reversible.
```

**This is the difference between working IN the machine and working ON the machine.**
...
- **Source:** Website (Bima Sakhi / Agent recruitment pages), referral, offline campaign, walk-in
- **Data captured:** Name, mobile, city, age, education, current occupation, motivation
- **Status:** `lead_captured`
- **Auto-actions:** CRM entry, WhatsApp template, email sequence start
- **This is the ONLY stage currently built.** Everything below is new.

#### Stage 2: Pre-Screening & Qualification
- **DO action:** Personal call within 24 hours
- **Eligibility check:**
  - Age: 18+ (Bima Sakhi: 18-50 preferred)
  - Education: 10th pass minimum (for regular agent: graduation preferred)
  - Location: Does the area need more agents?
  - Motivation: Why do they want to become an agent? (red flags: "just need a job")
  - Background: No criminal record, no existing IRDA license conflicts
- **Status:** `qualified` / `disqualified` / `needs_followup`
- **Disqualification logging:** Reason stored (for future campaign targeting improvement)
- **Admin UI action:** DO marks candidate as qualified after call, logs call notes

#### Stage 3: IC-38 Exam Preparation
- **DO action:** Distribute study material, form study group, assign mentor
- **Tracking:**
  - Study material shared: Yes/No + date
  - Study group assigned: group name/ID
  - Mock tests taken: count + scores
...
#### Stage 6: LIC Appointment
- **Process:**
  - Agency agreement signing (physical at branch)
  - Agent code generation by LIC
  - Branch assignment
...
#### Stage 7: Onboarding & Initial Training
- **Training modules tracked:**
  - Product training (LIC plan types: term, endowment, pension, ULIP, health)
  - Sales training (prospecting, need analysis, presentation, objection handling, closing)
  - Compliance training (KYC norms, IRDA guidelines, ethical selling)
  - Digital tools training (LIC apps, online proposal submission)
...
- **Milestones tracked:**
  - First 10 prospects identified (names listed)
  - First prospect meeting completed
...
CREATE TABLE agent_pipeline (
    ...
    -- Identity
    lead_id UUID REFERENCES crm_leads(id),     -- link to original lead
    candidate_name TEXT NOT NULL,
    candidate_mobile TEXT NOT NULL,
    candidate_email TEXT,
    candidate_city TEXT,
    candidate_locality TEXT,
    candidate_age INTEGER,
    candidate_education TEXT,
    candidate_occupation TEXT,
    candidate_gender TEXT CHECK (candidate_gender IN ('male','female','other')),
    agent_type TEXT CHECK (agent_type IN ('bima_sakhi','regular_agent')),
    
    -- Pipeline Stage
    current_stage TEXT NOT NULL DEFAULT 'lead_captured'
        CHECK (current_stage IN (
            'lead_captured', 'pre_screening', 'qualified', 'disqualified',
            'preparing_ic38', 'exam_registered', 'exam_attempted', 
            'exam_passed', 'exam_failed',
            'documents_collecting', 'documents_submitted', 'license_received',
            'appointment_pending', 'appointed',
            'in_training', 'activating', 'active_agent',
            'dropped_out', 'on_hold'
        )),
    stage_entered_at TIMESTAMPTZ DEFAULT NOW(),
    
    -- Stage 2: Pre-screening
    screening_call_date TIMESTAMPTZ,
    screening_call_notes TEXT,
    eligibility_status TEXT CHECK (eligibility_status IN ('eligible','not_eligible','needs_review')),
    disqualification_reason TEXT,
    
    -- Stage 3: IC-38 Prep
    study_material_shared BOOLEAN DEFAULT FALSE,
    study_material_shared_date DATE,
    study_group_name TEXT,
    mock_tests_taken INTEGER DEFAULT 0,
    last_mock_score INTEGER,
    exam_readiness TEXT CHECK (exam_readiness IN ('not_ready','preparing','ready')),
    
    -- Stage 4: IC-38 Exam
    exam_registration_date DATE,
    exam_date DATE,
    exam_center TEXT,
    exam_result TEXT CHECK (exam_result IN ('not_attempted','pass','fail')),
    exam_score INTEGER,
    exam_attempts INTEGER DEFAULT 0,
    
    -- Stage 5: IRDA License
    documents_checklist JSONB DEFAULT '{}',  -- {"photo": true, "pan": false, ...}
    documents_submitted_date DATE,
    irda_license_number TEXT,
    irda_license_valid_from DATE,
    irda_license_valid_until DATE,
    
    -- Stage 6: LIC Appointment
    agent_code TEXT,                          -- Official LIC agent code
    branch_name TEXT,
    appointment_date DATE,
    agreement_number TEXT,
    do_assigned TEXT,                         -- Development Officer name
    
    -- Stage 7: Training
    training_modules JSONB DEFAULT '{}',     -- {"product": "completed", "sales": "in_progress", ...}
    training_completed_date DATE,
    market_assignment TEXT,                   -- Assigned locality/area
    
    -- Stage 8: Activation
    first_prospect_date DATE,
    first_proposal_date DATE,
    first_policy_date DATE,
    first_policy_premium NUMERIC,
    first_policy_plan TEXT,
    policies_in_first_90_days INTEGER DEFAULT 0,
    activation_date DATE,                    -- When they hit the activation threshold
    
    -- Meta
    lead_source TEXT,                        -- website / referral / campaign / walk_in
    referral_source TEXT,                    -- If referral, who referred?
    dropout_reason TEXT,                     -- If dropped out, why?
    dropout_date DATE,
    notes TEXT,                              -- DO's running notes
    assigned_do UUID,                        -- Development Officer managing this candidate
    
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_pipeline_stage ON agent_pipeline(current_stage);
CREATE INDEX idx_pipeline_mobile ON agent_pipeline(candidate_mobile);
CREATE INDEX idx_pipeline_city ON agent_pipeline(candidate_city);
CREATE INDEX idx_pipeline_type ON agent_pipeline(agent_type);
```

### Pipeline Stage History (Every Transition Logged)

```sql
CREATE TABLE agent_pipeline_history (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    pipeline_id UUID REFERENCES agent_pipeline(id) ON DELETE CASCADE,
    from_stage TEXT NOT NULL,
    to_stage TEXT NOT NULL,
    changed_by TEXT NOT NULL,            -- DO email or 'system'
    change_reason TEXT,
    metadata JSONB,                       -- Extra data about this transition
    created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_pipeline_history ON agent_pipeline_history(pipeline_id, created_at DESC);
```
...
|-------|---------|--------|
| Stalled candidate | Same stage for >30 days | Admin notification + suggested follow-up |
| Exam approaching | Exam date in 3 days | WhatsApp reminder to candidate |
| IC-38 failed | Exam result = fail | Auto-schedule re-prep, motivational message |
| License expiring | IRDA license within 60 days of expiry | Admin alert + renewal workflow |
| Activation at risk | 60 days appointed, 0 policies | URGENT flag for DO |
| Dropout detected | No activity for 45 days after qualifying | "Win back" campaign trigger |
...
- **Admin Control (Section 32):** Pipeline is a new admin section, fully controllable
...
```
DAILY:    Did the agent meet prospects today? How many calls? Any proposals?
WEEKLY:   This week's production (new policies + premium collected). On track for monthly target?
MONTHLY:  Total policies, total premium, persistency ratio, product mix
QUARTERLY: Performance review, training needs assessment, club qualification check
ANNUALLY: MDRT/COT qualification, license renewal, career development discussion
```

### Active Agent Data Model

```sql
CREATE TABLE active_agents (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    
    -- Identity (linked from pipeline)
    pipeline_id UUID REFERENCES agent_pipeline(id),
    agent_code TEXT UNIQUE NOT NULL,        -- Official LIC agent code
    agent_name TEXT NOT NULL,
    ...
    -- Assignment
    city TEXT,
    localities TEXT[],                       -- Assigned working areas
    branch_name TEXT,
    do_id UUID,                              -- Development Officer
    
    -- Status
    status TEXT NOT NULL DEFAULT 'active'
        CHECK (status IN ('active','dormant','under_review','suspended','terminated')),
    activation_date DATE,
    
    -- Current Period Stats (updated monthly)
    current_month_policies INTEGER DEFAULT 0,
    current_month_premium NUMERIC DEFAULT 0,
    current_month_meetings INTEGER DEFAULT 0,
    current_month_proposals INTEGER DEFAULT 0,
    
    -- Lifetime Stats
    total_policies INTEGER DEFAULT 0,
    total_premium NUMERIC DEFAULT 0,
    persistency_13_month NUMERIC DEFAULT 0,  -- 13th month persistency %
    persistency_25_month NUMERIC DEFAULT 0,  -- 25th month persistency %
    persistency_61_month NUMERIC DEFAULT 0,  -- 61st month persistency %
    
    -- Product Mix (lifetime counts)
    term_policies INTEGER DEFAULT 0,
    endowment_policies INTEGER DEFAULT 0,
    pension_policies INTEGER DEFAULT 0,
    ulip_policies INTEGER DEFAULT 0,
    health_policies INTEGER DEFAULT 0,
    
    -- Club Qualifications
    mdrt_qualified BOOLEAN DEFAULT FALSE,
    mdrt_year INTEGER,
    cot_qualified BOOLEAN DEFAULT FALSE,     -- Court of the Table
    tot_qualified BOOLEAN DEFAULT FALSE,     -- Top of the Table
    chairmans_club BOOLEAN DEFAULT FALSE,
    
    -- Training
    last_training_date DATE,
    training_hours_this_year NUMERIC DEFAULT 0,
    mandatory_training_complete BOOLEAN DEFAULT FALSE,
    ...
    -- Meta
    notes TEXT,
    tags TEXT[],                              -- custom tags: "star_performer", "needs_attention"
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_agents_status ON active_agents(status);
CREATE INDEX idx_agents_code ON active_agents(agent_code);
CREATE INDEX idx_agents_do ON active_agents(do_id);
```
...
CREATE TABLE agent_activity_log (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    agent_id UUID REFERENCES active_agents(id) ON DELETE CASCADE,
    ...
    policies_closed INTEGER DEFAULT 0,
    premium_collected NUMERIC DEFAULT 0,
    
    -- Details
    prospect_names TEXT[],                   -- Names of people met
    meeting_notes TEXT,
    plan_for_tomorrow TEXT,
    
    -- Source
    logged_by TEXT NOT NULL,                  -- 'agent' or 'do'
...
### Agent Monthly Performance Snapshot
...
CREATE TABLE agent_monthly_performance (
    ...
    total_premium NUMERIC DEFAULT 0,
    new_business_premium NUMERIC DEFAULT 0,
    ...
    -- Ranking
    rank_in_team INTEGER,                    -- Rank among DO's agents
    rank_in_branch INTEGER,                  -- Rank in branch
...
- Performance chart (last 12 months trend)
- Activity log (daily)
- Policy list (all policies sold)
- Customer list (all customers)
- Training history
- Club qualification progress (MDRT calculator)
- Notes from DO

### Motivation & Competition System
...
| Compliance Item | Frequency | Consequence of Non-Compliance |
|----------------|-----------|------------------------------|
| IRDA license renewal | Every 3 years | Cannot sell policies, license lapse |
| Mandatory training hours | 25 hours/year | License renewal blocked |
| KYC audit | Annual | Compliance violation flag |
| Premium collection audit | Quarterly | Misappropriation risk |
| Policy servicing standards | Ongoing | Customer complaints |

**2. Dormancy Detection & Reactivation:**

```
DORMANCY DEFINITION:
  - Zero new policies for 3 consecutive months
  - Zero customer meetings logged for 30 days
  - Zero activity in system for 45 days
...
```
TERMINATION TYPES:
  - Voluntary: Agent resigns (better opportunity, personal reasons)
  - Involuntary: LIC terminates (compliance violation, fraud, non-performance)
  - Lapse: IRDA license expired, not renewed

TERMINATION PROCESS:
  1. Log termination reason + date
  2. Identify all ORPHAN POLICIES (customers without servicing agent)
...
> *"Jab agent terminate hota hai, uske customers orphan ho jaate hain. Kisi ko pata nahi hota ki unki policy ka kya hoga. Ye customer loss ka sabse bada reason hai."*

```sql
-- When an agent terminates, this query finds all orphan policies:
-- SELECT * FROM customer_policies 
-- WHERE servicing_agent_id = [terminated_agent_id]
-- AND policy_status = 'inforce'
...
```sql
-- Add to active_agents table:
ALTER TABLE active_agents ADD COLUMN IF NOT EXISTS dormancy_start_date DATE;
ALTER TABLE active_agents ADD COLUMN IF NOT EXISTS dormancy_reason TEXT;
ALTER TABLE active_agents ADD COLUMN IF NOT EXISTS termination_date DATE;
ALTER TABLE active_agents ADD COLUMN IF NOT EXISTS termination_reason TEXT;
ALTER TABLE active_agents ADD COLUMN IF NOT EXISTS termination_type TEXT 
    CHECK (termination_type IN ('voluntary','involuntary','license_lapse'));
ALTER TABLE active_agents ADD COLUMN IF NOT EXISTS reactivation_attempts INTEGER DEFAULT 0;
ALTER TABLE active_agents ADD COLUMN IF NOT EXISTS last_reactivation_date DATE;
ALTER TABLE active_agents ADD COLUMN IF NOT EXISTS compliance_status TEXT DEFAULT 'compliant'
    CHECK (compliance_status IN ('compliant','warning','violation','under_review'));

-- Compliance tracking table
CREATE TABLE agent_compliance_log (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    agent_id UUID REFERENCES active_agents(id) ON DELETE CASCADE,
    compliance_type TEXT NOT NULL,            -- 'irda_renewal', 'training_hours', 'kyc_audit'
    status TEXT NOT NULL,                     -- 'compliant', 'due_soon', 'overdue', 'completed'
    due_date DATE,
    completed_date DATE,
    notes TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW()
...
CREATE TABLE customers (
    ...
    -- Relationship
    servicing_agent_id UUID REFERENCES active_agents(id),
    relationship_start_date DATE,
    lead_source TEXT,                        -- website / referral / campaign / walk_in
    referred_by UUID REFERENCES customers(id),  -- If referral, which customer referred?
    
    -- Lifecycle
    lifecycle_stage TEXT DEFAULT 'prospect'
        CHECK (lifecycle_stage IN (
            'prospect', 'first_buyer', 'multi_policy', 
            'mature', 'lapsed_risk', 'referral_source', 
            'claims_active', 'not_interested', 'archived'
        )),
    
    -- Aggregate Policy Data (computed)
    total_policies INTEGER DEFAULT 0,
    total_annual_premium NUMERIC DEFAULT 0,
    active_policies INTEGER DEFAULT 0,
    lapsed_policies INTEGER DEFAULT 0,
    
    -- Engagement
    last_contact_date DATE,
    last_contact_type TEXT,                  -- call / meeting / whatsapp / email
    next_scheduled_contact DATE,
    contact_preference TEXT DEFAULT 'phone'
        CHECK (contact_preference IN ('phone','whatsapp','email','in_person')),
    
    -- Cross-sell Signals
    has_term_plan BOOLEAN DEFAULT FALSE,
    has_endowment BOOLEAN DEFAULT FALSE,
    has_pension BOOLEAN DEFAULT FALSE,
    has_health BOOLEAN DEFAULT FALSE,
    has_child_plan BOOLEAN DEFAULT FALSE,
    cross_sell_opportunity TEXT,              -- AI-suggested next product
    
    -- Satisfaction
    nps_score INTEGER,                       -- Net Promoter Score (1-10)
...
CREATE INDEX idx_customers_agent ON customers(servicing_agent_id);
...
CREATE TABLE customer_policies (
    ...
    plan_name TEXT NOT NULL,                  -- "Jeevan Shanti", "LIC Tech Term"
    plan_type TEXT NOT NULL                   -- term / endowment / pension / ulip / health
        CHECK (plan_type IN ('term','endowment','pension','ulip','health','child','money_back','whole_life')),
    sum_assured NUMERIC,
    annual_premium NUMERIC,
    premium_frequency TEXT DEFAULT 'annual'   -- annual / half_yearly / quarterly / monthly
        CHECK (premium_frequency IN ('annual','half_yearly','quarterly','monthly')),
    ...
    -- Status
    policy_status TEXT DEFAULT 'inforce'
        CHECK (policy_status IN ('inforce','lapsed','paid_up','matured','surrendered','claimed','cancelled')),
    ...
    -- Rider Details
    riders JSONB DEFAULT '[]',               -- [{"type": "accident", "sum": 500000}]
    
    -- Commission
    first_year_commission NUMERIC,
    renewal_commission NUMERIC,
    
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_policies_customer ON customer_policies(customer_id);
CREATE INDEX idx_policies_status ON customer_policies(policy_status);
CREATE INDEX idx_policies_due ON customer_policies(next_premium_due_date);
```
...
```
Policy anniversary (every year):
  - WhatsApp: "Policy ko [X] saal ho gaye! Review meeting schedule karein?"
  - Agent action: Schedule annual review meeting
  - Cross-sell check: Does customer need additional coverage?

Birthday:
  - WhatsApp greeting: "Happy Birthday [name] ji! LIC family ki taraf se shubhkaamnayein"
  - Agent action: Use birthday as relationship touchpoint

Marriage anniversary (if known):
  - Gentle touchpoint + family coverage suggestion
```
...
### Admin UI: Customer Dashboard `/admin/agency/customers`
...
Every lead is either:

```
TYPE A: CUSTOMER LEAD (wants to buy a policy)
  Sub-types:
    - Term plan buyer
    - Endowment buyer
    - Pension/annuity buyer
    - Child plan buyer
    - Health plan buyer
    - General inquiry (not sure what they want)

TYPE B: RECRUITMENT LEAD (wants to become an agent)
  Sub-types:
    - Bima Sakhi candidate (female, specific program)
    - Regular agent candidate (male/female)
    - IC-38 help seeker (may become agent)

TYPE C: SERVICE LEAD (existing customer needs service)
  Sub-types:
...
TYPE D: UNDECIDED (needs qualification)
  - Came via general inquiry, not clear intent
  - Needs qualification call to categorize
```

### Universal Lead Table Enhancement

```sql
-- Extend the existing crm_leads table:
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS lead_source_type TEXT 
    CHECK (lead_source_type IN (
        'website_organic', 'website_download', 'website_contact',
        'referral_customer', 'referral_agent', 'referral_do',
        'campaign_digital', 'campaign_offline', 'campaign_whatsapp',
        'social_media_dm', 'walk_in', 'cold_outreach', 'other'
    ));
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS lead_category TEXT
    CHECK (lead_category IN ('customer','recruitment','service','undecided'));
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS lead_sub_type TEXT;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS referral_source_name TEXT;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS referral_source_id UUID;     -- customer or agent who referred
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS campaign_name TEXT;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS campaign_id TEXT;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS assigned_agent_id UUID REFERENCES active_agents(id);
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS follow_up_sla_minutes INTEGER;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS first_contact_at TIMESTAMPTZ;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS sla_met BOOLEAN;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS conversion_status TEXT DEFAULT 'open'
    CHECK (conversion_status IN ('open','contacted','interested','proposal_sent',
                                  'converted','lost','disqualified','nurturing'));
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS conversion_date DATE;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS lost_reason TEXT;
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS pipeline_id UUID;             -- links to agent_pipeline if recruitment
ALTER TABLE crm_leads ADD COLUMN IF NOT EXISTS customer_id UUID;             -- links to customers if converted
```
...
**Method 2: Manual Entry by DO `/admin/agency/leads/new`**
```
For leads that come via referral, walk-in, campaign, or cold outreach:
  DO opens the "New Lead" form in admin
  Fields:
    Name, Mobile, City, Locality
    Source: [dropdown of all source types]
    Category: [Customer / Recruitment / Service / Undecided]
...
**Method 3: Bulk Import `/admin/agency/leads/import`**
```
For offline campaign leads (seminar, event):
...
CREATE TABLE lead_follow_ups (
    ...
    follow_up_number INTEGER NOT NULL,       -- 1st, 2nd, 3rd...
    scheduled_at TIMESTAMPTZ NOT NULL,
    completed_at TIMESTAMPTZ,
    
    contact_method TEXT NOT NULL,             -- call / whatsapp / email / meeting / visit
    outcome TEXT,                             -- interested / not_available / callback / not_interested / converted
    notes TEXT,
    next_action TEXT,                         -- What to do next?
    next_follow_up_date DATE,
    
    done_by TEXT NOT NULL,                    -- Agent or DO who did the follow-up
    created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_followups_lead ON lead_follow_ups(lead_id);
CREATE INDEX idx_followups_scheduled ON lead_follow_ups(scheduled_at);
```
...
CREATE TABLE lead_campaigns (
    ...
    campaign_type TEXT NOT NULL
        CHECK (campaign_type IN ('digital_ad','offline_event','whatsapp_broadcast',
                                  'email_campaign','social_media','referral_program','other')),
    
    -- Details
    start_date DATE,
    end_date DATE,
    budget NUMERIC,
    target_audience TEXT,                     -- "Women 25-40, Delhi NCR"
    description TEXT,
    ...
    cost_per_lead NUMERIC,                   -- budget / total_leads
    cost_per_conversion NUMERIC,             -- budget / total_converted
    roi NUMERIC,                             -- revenue generated vs budget
...
The pagegen system generates content. But does it LEARN?
...
CYCLE:
  ...
  5. System auto-generates MORE content in winning patterns
  6. System auto-flags losing content for re-optimization
...
```
CURRENT:  quality_score = word count + heading structure + keyword density
NEEDED:   content_intelligence_score = quality_score 
          + actual GSC performance (position, CTR)
          + conversion rate (did the page generate leads?)
...
The lead system captures leads. But does it SCORE them intelligently?

**AI-Based Lead Scoring Model:**
```
CURRENT:  lead_score = basic formula (source weight + intent weight)
NEEDED:   intelligent_lead_score = 
          Source quality (which sources actually convert?)
          + Behavioral signals (visited 3+ pages? downloaded PDF? spent 5+ min?)
          + Profile match (age/location/income match for LIC products?)
          + Timing signals (visited at 9 PM on Sunday = research mode = high intent)
...
The agent system tracks agents. But does it make SMART decisions about them?
...
NEEDED:   Intelligent matching = 
          ...
          + Track record (agent's conversion rate for this lead type)
          + Current capacity (agent has 5 pending leads = overloaded, skip)
          + Performance trend (agent's numbers are rising = give more leads)
...
The 5 ecosystems (Sections 33-37) are defined independently. But the REAL power is in cross-ecosystem insights.
...
### Layer 5: Automation Rule Engine (CEO-Configurable)

The CEO should be able to create business rules WITHOUT code changes:

**Rule Engine Concept:**
```
ADMIN UI: /admin/intelligence/rules

RULE FORMAT:
  IF [condition] THEN [action] ELSE [fallback]

EXAMPLE RULES (CEO creates from admin panel):

Rule 1: "Hot Lead Auto-Assignment"
  IF lead_score > 80 AND lead_city = "Delhi"
  THEN assign_to_agent("Agent-47") AND send_whatsapp("New hot lead!") AND set_sla("4 hours")
  ELSE assign_to_agent("round-robin") AND set_sla("24 hours")

Rule 2: "Dormancy Alert"
  IF agent.policies_this_month = 0 AND agent.days_since_last_activity > 14
  THEN send_alert_to_ceo("Agent {name} trending dormant") AND create_task("Retention call")

Rule 3: "Content Auto-Optimization"
  IF page.gsc_impressions > 1000 AND page.gsc_ctr < 2%
  THEN add_to_reoptimization_queue() AND set_priority("high")
  NOTE: "High impressions but low CTR = title/meta needs work"

Rule 4: "Premium Reminder Cascade"
  IF customer.premium_due_in_days <= 15
  THEN send_whatsapp_reminder() AND alert_servicing_agent()
  IF customer.premium_due_in_days <= 3 AND NOT customer.premium_paid
  THEN escalate_to_ceo() AND send_urgent_reminder()

Rule 5: "Agent Reward Trigger"
  IF agent.policies_this_month >= 5
  THEN add_to_leaderboard_highlight() AND send_congratulations() AND badge("Star Performer")
```
...
CREATE TABLE automation_rules (
    ...
    trigger_type TEXT NOT NULL
        CHECK (trigger_type IN ('event','schedule','threshold','manual')),
    trigger_entity TEXT NOT NULL
        CHECK (trigger_entity IN ('lead','agent','customer','content','page','policy')),
    condition_json JSONB NOT NULL,        -- Structured condition: {"field": "lead_score", "operator": ">", "value": 80}
    
    -- Action
    action_type TEXT NOT NULL
        CHECK (action_type IN ('assign','alert','create_task','send_message','update_status',
                                'add_to_queue','escalate','badge','custom')),
    action_config JSONB NOT NULL,         -- {"target": "agent-47", "channel": "whatsapp", "message_template": "hot_lead"}
    
    -- Fallback
    else_action_type TEXT,
    else_action_config JSONB,
    
    -- Control
    is_active BOOLEAN DEFAULT true,
    priority INTEGER DEFAULT 0,           -- Higher = runs first when multiple rules match
...
### Layer 6: System Health Intelligence

The system should monitor ITSELF and alert when things go wrong:
...
PHASE 20 BUILD ORDER:
  Step 1: automation_rules table + basic rule engine
  Step 2: Lead intelligence scoring (upgrade lead_score)
  Step 3: Content performance feedback loop (GSC + GA4 integration)
  Step 4: Agent-lead intelligent matching
  Step 5: Cross-ecosystem query engine
  Step 6: CEO morning brief automation
  Step 7: Self-monitoring + anomaly detection
  Step 8: Agent performance prediction
  Step 9: Conversion prediction model
  Step 10: Full rule engine UI (/admin/intelligence/rules)
```
...
Before diving into governance, it's important to understand WHERE the system is in its maturity journey:
...
### External System Contracts (Formal)
...
This section becomes Phase 21. It should be Priority A (MANDATORY) because without it, the system is vulnerable to any external failure.

```
PHASE 21 BUILD ORDER:
  Step 1: sla_snapshots table + basic health check endpoints
  Step 2: dead_letter_queue table + DLQ consumer job
  Step 3: alert_deliveries table + multi-channel alert dispatch
  Step 4: Circuit breaker middleware for all external API calls
  Step 5: System mode management (normal/degraded/safe/maintenance)
  Step 6: Supabase contract enforcement (timeout, retry, fallback)
  Step 7: QStash contract enforcement (pending_jobs backup queue)
  Step 8: Zoho contract enforcement (local-first, sync later)
  Step 9: Gemini contract enforcement (model fallback chain)
  Step 10: /admin/system/ health dashboard
...
**This is not a nice-to-have. This is the difference between a startup and a system.**

### The Solution: `docs/` as System Memory
...
#### Mandatory Fields (Every Doc)

| Field | Required | Purpose |
|---|---|---|
| Title | Yes | Clear, descriptive title |
| Date | Yes | When this happened |
...
| Remaining | If applicable | What's still open |
| Cross-References | Yes | Links to related audits/fixes/incidents |
...
DOCS ? DOCS:
  ...
  Migrations reference the fix or feature that needed them.
```
...
DURING WORK:
  ...
  6. Update docs/INDEX.md with the new entry
  7. Cross-reference in the bible if relevant

AFTER COMPLETING WORK:
  8. Verify docs/ file has all mandatory fields
  9. Ensure cross-references are bidirectional
  10. Update audit report if findings changed
  11. Report completion to CEO WITH doc reference
```

### Why This Matters (Real Impact)

| Without System Memory | With System Memory |
|---|---|
...
This is a **file-based** system. No database tables needed. The `docs/` folder IS the system memory. It lives in the git repo, versioned, permanent, and accessible to every agent.

### Bible Integration

This section creates the documentation layer that CONNECTS all other sections:

- **Rule 25** defines the mandate (MUST document everything)
- **Section 40** (this section) defines the architecture and standards
- **Phase 22** defines the implementation sequence
- **docs/INDEX.md** is the operational entry point
...
- `alertSystem.js` has `sendWhatsAppAlert()` but `ALERT_WHATSAPP_WEBHOOK` = NOT SET
- `utils/whatsapp.js` = just a `wa.me` URL builder (click-to-chat only)
- Telegram = ZERO code, ZERO integration
- Zoho Cliq = Bible's designated primary channel, ZERO implementation
- Result: All alerts go to DB only. CEO is blind to system events.
...
- Delivery confirmation: Read receipt API tracking
- Rate limits: Respect Meta's messaging limits

#### Telegram Bot (P1)
- Bot creation via @BotFather
- Use cases: System alerts, internal team notifications, daily reports
- Channels: CEO private channel, CTO alerts channel, Agent group
...
- Bot: BimaSakhi System Bot for automated messages

### Delivery Rules (Rule 27 Enforcement)
...
VERIFICATION:
  ...
  - channel_used (which channel delivered)
...
### Phase: 23 | Priority: A | Depends on: None
...
- Search by filename, alt text, associated content
- Filter by type (hero/thumbnail/og), date, content association
- Bulk upload (drag & drop multiple)
- Bulk delete with confirmation
- Image preview with metadata editing
- Usage tracking (which pages use this image)
- Orphan detection (images not linked to any content)

### Phase: 24 | Priority: A | Depends on: None
...
- No way to see what pages exist for a given city
...
- See ALL content in one view (currently impossible)
- Search across all content types
- Filter by content type, status, city, date
- Create new content of any type from this dashboard
- Quick actions: publish, unpublish, delete, edit
...
- CEO sees geo data but can't modify it
...
- Assign agent to city/locality
- Edit city/locality names and configuration

### Phase: 27 | Priority: A | Depends on: Phase 5 fixes
...
Every significant decision (new phase, feature, architecture change) MUST be documented:
...
| Decision | Problem | What Should Have Happened |
|----------|---------|--------------------------|
| Build Phase 3 before Publish Pipeline | Images have no pages to live on | Decision doc would have caught dependency gap |
| Build Phase 21 enterprise-grade infra | Over-engineered for 1-user system | Decision doc would have caught scale mismatch |
| Mark phases COMPLETE with known bugs | Constitution violation | Decision doc would have forced honest status |
| Skip WhatsApp/Telegram integration | CEO blind to system events | Decision doc would have caught channel gap |

### Rule: 32 | Governance: Always active (applies to ALL phases)
...
TRUTH RULE 2: UPDATE ON EVERY CHANGE
  ...
  1. Update the truth section FIRST
  2. Update the Execution Priority Matrix
  3. These two MUST always agree

TRUTH RULE 3: NO STALE DATA
  "Next Action" must reflect actual current state.
  "Last Updated" date must be the actual last update date.
  Phase scores must match the latest audit.
...
TRUTH RULE 5: VERSION HISTORY
  ...
  | Date | Change | By |
  | April 19, 2026 | Constitution added, Rules 26-33, Sections 41-49, phase corrections | CTO |
```

### Rule: 47 | Governance: Always active
...
STEP 3: WORKER ASSIGNMENT
  ...
  Worker reads bible + relevant sections + audit findings
  Worker claims sub-phase (marks IN PROGRESS)

STEP 4: INDEPENDENT EXECUTION
  Each worker builds their sub-phase
  Workers do NOT touch files outside their scope
...
STEP 6: CEO REVIEW
  CTO presents completed phase to CEO
  CEO tests in production
  CEO marks COMPLETE or requests changes
```

### Worker Contract (Every Worker MUST Follow)
...
- Fix BEFORE build. No new features until existing bugs are resolved.
- Follow audit part/sub-part structure exactly.
- Each fix stage maps to audit findings.
- Mosaic execution: independent fixes run in parallel.
...
### STAGE 2: NAVIGATION & ADMIN ACCESS (Audit Part 3)

> **Target: CEO can find and use ALL admin pages**
...
### STAGE 5: GEO + BULK PLANNER UPGRADES (Audit Part 3, Pages 2-3)

> **Target: Fix Phase 4 + Phase 5 from 7/10 to 9/10**
...
### STAGE 6: ADMIN PAGE ENHANCEMENTS (Audit Part 3, Pages 4-6)

> **Target: Make admin pages actionable, not just viewable**
...
### STAGE 7: BIBLE CONSISTENCY (Audit Part 5)

> **Target: Single source of truth, zero contradictions**
...
### Execution Timeline (Mosaic Model)
...
PARALLEL STREAM 5 (Geo + Planner):
  Stage 5a-5e (mostly parallel)
  Worker: assigned developer

PARALLEL STREAM 6 (Admin Enhancements):
  Stage 6a-6f (all parallel)
  Worker: assigned developer

JOIN PHASE:
  CTO verifies all 6 streams
  Integration testing across streams
  CEO review + approval

TOTAL: 7 stages, 35 sub-fixes, 4 parallel streams + 1 sequential + 1 parallel
```
...
NICE TO HAVE THIS SPRINT:
  1c-1f (media improvements)
  6a-6f (admin enhancements)
```
...
- Intent-specific thank-you pages (`/thank-you/[funnel]`)
- CRM bifurcation tags + Zoho CRM pipeline mapping
- Contact page intent-router redesign
...
**Phase 10: Analytics Stack Setup (parallel with Phase 1)**
- GTM container creation + tag configuration (GA4, events)
- GSC property setup + sitemap submission
- Custom GA4 events for all key actions
- GT Matrix baseline test run after Phase 1 deployment

**Phase 11: Bilingual Engine (during Phase 2)**
- Hindi content generation in pagegen worker (Pass 2)
- DB column additions for `_hi` variants
- Language-aware rendering in `GeneratedPageTemplate`
- Draft editor language tabs

**Phase 12: Intelligence Layer + Social Engine (after Phase 6)**
- `page_performance_snapshots` table + GSC API integration
- AI pattern analyzer job (bi-weekly scheduled)
- `admin_insights` table + `/admin/ccc/insights` page
- Re-optimization engine + "Revive" flag in draft queue
- Social content auto-generation on page approval
...
- Safe Mode: one-click emergency system pause
...
- Pipeline conversion metrics dashboard
- Integration: website recruitment leads auto-create pipeline entries

**Phase 16: Active Agent Management System (after Phase 15)**
- `active_agents` table + `agent_activity_log` + `agent_monthly_performance`
- Agent dashboard with production tracking (`/admin/agency/agents`)
- Individual agent profile pages
- Activity logging (daily calls, meetings, proposals)
- Leaderboard + competition system
- Club qualification tracker (MDRT/COT/TOT)
...
- Termination process with orphan policy identification
- Orphan policy reassignment tool (`/admin/agency/orphans`)
- IRDA license renewal tracking + alerts
...
- Lapsed policy report + maturity report
- Customer profile pages with full policy view

**Phase 19: Universal Lead Management Hub (after Phase 18)**
- Extend `crm_leads` with source classification columns
- `lead_follow_ups` table + `lead_campaigns` table
- Manual lead entry form for referral/walk-in/campaign leads
- CSV bulk import for offline campaign leads
- Lead routing intelligence (geo, product, load, performance-based)
- Follow-up SLA tracking with alerts
- Source ROI dashboard (which sources convert best)
- Campaign creation + tracking + ROI comparison
...
- Cross-link all existing docs to bible sections
- Agent onboarding protocol: "Read INDEX.md first" mandatory rule
- Template files for each doc type (audit, fix, feature, incident, decision, migration)
- Bible Section 40 + Rule 25 define the standards (already added)
...
### Mosaic Execution Model (How Phases Execute)
...
> *"Code developers ke liye hota hai. Control business owner ke liye hota hai."*

You should not need a developer to:
- Turn off pagegen when something goes wrong
- Change the quality score threshold
- See why a page failed to generate
- Check which file is causing an error
...
> *"Tum system ke andar nahi kaam karoge. System tumhare control mein kaam karega."*
...
**Admin Route:** `/admin/control/features`
...
CONTENT QUALITY THRESHOLDS
  Auto-reject if word count below: [300] words
  Flag for review if word count below: [400] words
  Auto-approve if quality score above: [8.0] (only if auto-approve is ON)
...
PUBLISHING RULES
  ...
  Sitemap drip rate: [50] new URLs per day

GENERATION RULES
  Max queue items per bulk job: [500]
  Generation rate per hour: [50] items
  Max retries per failed job: [3]
  Min internal links before publish allowed: [1]

LEAD RULES
  Hot lead score threshold (Cliq alert): [100]
  Warm lead score threshold (WhatsApp): [70]
...
#### Sub-Feature 1: File Viewer

Browse the project file structure from the admin. Click any file to see its contents (syntax highlighted, read-only).

```
/admin/system/code/viewer

File Tree (left panel):            Code View (right panel):
  app/                              // lib/queue/publisher.js
...
For any key module, show which other files use it / call it / import it.

```
/admin/system/code/dependencies
...
Search the codebase by:
- Function name (e.g., `enqueuePageGeneration`)
- API route (e.g., `/api/jobs/pagegen`)
- Table name (e.g., `generation_queue`)
- Text pattern (e.g., `queueId`)
...
- Source: All | pagegen | lead | publish | queue | schedule | GSC | email | whatsapp
...
### Advanced Features (Built Progressively)
...
CREATE TABLE admin_audit_log (
    ...
    action TEXT NOT NULL,        -- 'feature_toggle' | 'draft_approved' | 'page_unpublished' | etc.
    entity_type TEXT NOT NULL,   -- 'feature_flag' | 'draft' | 'page' | 'bulk_job' | 'workflow_config'
    entity_id TEXT,              -- slug, UUID, or key of the changed entity
    old_value JSONB,             -- what it was before
    new_value JSONB,             -- what it became
    ip_address TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
```
...
Every time a draft or live page is edited and saved, the previous version is stored:
...
CREATE TABLE content_version_history (
    ...
    snapshot JSONB NOT NULL,    -- full snapshot of content_drafts row at this point
    change_summary TEXT,        -- brief description of what changed
    created_at TIMESTAMPTZ DEFAULT NOW()
);
```

In the draft editor, a "Version History" tab shows all previous versions with a diff view (current vs. selected version). CEO can restore any previous version with one click.
...
### Safety Guardrails for the Admin Panel Itself

The admin panel is powerful. That power must be constrained by its own safety rules:

| Action | Safety Gate |
|--------|------------|
| Toggle SAFE MODE on/off | Double confirmation dialog: "Are you sure? This pauses all automation." |
| Delete any content | Soft delete only. "Move to Trash" first. Permanent delete = 2nd confirmation + audit logged. |
| Bulk approve / bulk publish | Show count first. "You are about to publish 47 pages. Confirm?" |
| Change auto_approve threshold | Warning: "Lowering this threshold will auto-publish more pages without review." |
| Access Code Visibility panel | Restricted to `super_admin` role only. Read-only enforced server-side. |
| View `.env` or secret files | Explicitly blocked. Returns 403. Not shown in file tree. |
| Re-trigger a failed job | Confirmation required. Idempotency check runs first (won't duplicate if already processed). |
| Delete a bulk job | Only allowed if job is in `planned` or `paused` state. Cannot delete `running` or `completed` jobs. |

**The principle:** Destructive or irreversible actions always require more friction than constructive actions. Easy to create, harder to destroy.

---

### Admin Panel Route Map (Complete)
...
CREATE TABLE IF NOT EXISTS admin_users (
    ...
    role TEXT NOT NULL DEFAULT 'editor'
        CHECK (role IN ('super_admin','editor','agent')),
...
CREATE INDEX idx_audit_user ON admin_audit_log(user_email);
CREATE INDEX idx_audit_action ON admin_audit_log(action);
CREATE INDEX idx_audit_created ON admin_audit_log(created_at DESC);

-- content_version_history (draft edit history)
...
| 10 | Analytics Stack | PARTIAL | 30% | Admin analytics exists. Full GTM/GA4/GSC proof remains absent. |
...
| 21 | External System Governance | PARTIAL | 65% | Zoho refresh + test lead passed. QStash publish passed, but current provider log lookup remains partial. Historical no-cleanup failed delivery rows remain preserved in `external_delivery_logs`, while the current live health window is healthy because recent delivery metrics are zero. |
...
Fresh 2026-05-01 live proof:
  ...
  - `/api/status`, `/api/test`, `/api/navigation`, `/sitemap.xml`, `/`, and `/why` returned `200`
  - `/api/status` reported version `47404cc`, `status=ok`, and `overall_health=HEALTHY`
  - Admin login returned `200` and issued a valid session cookie
  - `/api/admin/system/health` returned `overall_health=HEALTHY` with no hard failures
  - Authenticated admin reads for drafts, bulk, media, logs, audit log, feature flags, workflow config, users, geo cities, and system health all returned `200`
  - Direct Supabase REST reads for `system_control_config`, `feature_flags`, `system_alerts`, `job_dead_letters`, and `event_store` all returned `200`
  - Direct REST still shows historical failed C26 rows in `external_delivery_logs` for the missing-route proof events
  - `/api/admin/delivery-logs` now reports `delivery_failures_recent=0`, `delivery_stuck_count=0`, `delivery_success_rate=100`, and `delivery_terminal_recent=0`
  - Zoho OAuth refresh and test lead upsert both passed

Fresh 2026-05-01 warnings / remaining limits:
  - QStash publish passed, but current provider log lookup remains partial because the returned `/v2/logs` sample did not include the new message id
...
C22 STATUS: RESOLVED IN PRODUCTION
  ...
  - Authenticated `/api/admin/users` now returns 200.
  - Local users POST hardening patch remains a later cleanup deploy, not a blocker for C22.

1. C31 STATUS: RESOLVED IN PRODUCTION
  - Commit `7335ece` is live in `/api/status`.
  - Password-only login now fails with `400`.
...
8. C26 STATUS: CLOSED IN REQUESTED AUDITED SCOPE
  ...
  - No-cleanup persistence proof used deterministic missing-route failure: provider logs showed real `404 -> RETRY -> FAILED` lifecycle for `event_id=8dcd38ce-a056-4316-bd12-5738816e11d7`.
  - The proof artifact captured a retained failed delivery row after the 365-second wait with `provider_retry_count=2` and terminal provider state `FAILED`.
  - In the original April 27 no-cleanup proof window, `/api/admin/delivery-logs` metrics and `/api/admin/system/health` both reflected the retained failure.
  - In the May 1 forensic reconciliation, the historical failed rows still existed in `external_delivery_logs`, but current delivery metrics were zero and current live health was `HEALTHY` because delivery health uses a recent 24-hour window.
  - C26 closure remains valid as historical no-cleanup persistence proof. It is no longer the current live system mode.
