...
export const adminApi = {
    ...
    getLeads: () => fetcher(`${BASE_ADMIN_DATA}?action=leads-list`),
    ...
    getConfig: () => fetcher(`${BASE_ADMIN_DATA}?action=config-get`),
    saveConfig: (config) => fetcher(`${BASE_ADMIN_DATA}?action=config-save`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(config)
    }),
    getStats: () => fetcher(`${BASE_ADMIN_DATA}?action=stats`)
};
