...
export async function routeLeadToAgent(leadId) {
    ...
    const { data: eligibleAgents } = await supabase
        ...
        .eq('status', 'active'); // simplified rule - active agents
    ...
    // Weight: (ConversionRate * 60) + (100 - min(AssignedRecent, 100)) + ResponseBonus
    for (const agent of eligibleAgents) {
        ...
        let conversionRate = stats.leads_assigned > 0 ? (stats.leads_converted / stats.leads_assigned) * 100 : 50; // Default new agents to 50% fair chance
        let fairnessScore = Math.max(0, 100 - stats.leads_assigned); // Prioritize lower assignment counts
        let responseBonus = Math.max(0, 60 - stats.avg_response_time_minutes); // Up to 60 bonus points for fast SLAs
    ...
    const { error: assignErr } = await supabase
        ...
        .eq('id', leadId)
        .is('agent_id', null); // Concurrency protection - ensure it hasn't mapped yet
    ...
    await Promise.all([
        ...
        supabase.rpc('increment_agent_assignments', { agent_uid: bestAgent.agent_id }) // Assumes RPC created or we increment normally
    ]);

    // Fallback if RPC doesn't exist
    await supabase.from('agent_business_metrics')
        .update({ leads_assigned: supabase.rpc('leads_assigned + 1') }) // this is pseudocode; better logic if RPC missing
        .eq('agent_id', bestAgent.agent_id)
        .catch(() => {});
