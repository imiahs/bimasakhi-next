import { getAdminAccessDecision } from './adminAccessControl';
...
export function withAdminAuth(handler, allowedRoles = null) {
    return async function protectedHandler(request, context) {
        ...
        // 1. Verify admin session directly from the cookie or cron bearer token.
        // Middleware still performs the coarse access gate, but route handlers
        // should not depend on request-header mutation to recover the user.
        ...
        if (!accessDecision.allowed) {
            const roleCheck = requireRole(user, accessDecision.allowedRoles);
