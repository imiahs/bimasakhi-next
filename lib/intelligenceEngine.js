...
async function fetchCurrentSystemState() {
    ...
    const [failedRes, queueRes, totalLeadsRes, convLeadsRes, sourcesRes] = await Promise.all([
        supabase.from('failed_leads').select('id', { count: 'exact', head: true }).eq('retry_pending', false),
        ...
        supabase.from('leads').select('Lead_Source').limit(50) // Reduced sample limit for memory bounds
    ]);
    ...
    // Process top sources from sample
    const sourceMap = {};
    (sourcesRes.data || []).forEach(lead => {
