...
async function fetchCurrentSystemState() {
    ...
    const [failedRes, queueRes, totalLeadsRes, convLeadsRes, sourcesRes] = await Promise.all([
        supabase.from('failed_leads').select('*', { count: 'exact', head: true }).eq('retry_pending', false),
        ...
        supabase.from('leads').select('Lead_Source').limit(500) // Sample last 500 for fast aggregation
    ]);
