...
export async function runAlertScan() {
    ...
    if (resolvedRules.length > 0) {
        const { data, error } = await supabase
            ...
            .in('alert_type', resolvedRules)
            .eq('resolved', false)
            .select('id');

        if (!error) {
            autoResolved = data?.length || 0;
        }
    }
    ...
    try { await supabase.from('observability_logs').insert({
        ...
        source: 'alert_system',
        metadata: { results, auto_resolved: autoResolved },
    }); } catch (_) {}

    return { rules_checked: results.length, alerts_fired: fired.length, auto_resolved: autoResolved, results };
}
