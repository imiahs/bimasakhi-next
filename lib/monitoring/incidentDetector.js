...
export const runAnomalyScan = async () => {
    ...
    // 2. Detect Dead Workers (Heartbeat older than 10 mins but status is 'online')
    const { data: stalledWorkers } = await supabase
        .from('worker_health')
        .select('worker_name, last_heartbeat')
        .lte('last_heartbeat', tenMinsAgo)
        .eq('status', 'online');

    if (stalledWorkers && stalledWorkers.length > 0) {
        stalledWorkers.forEach(worker => {
            alertsToFire.push({
                alert_type: 'worker',
                severity: 'high',
                message: `WORKER STALLED: ${worker.worker_name} has missed its heartbeat for over 10 minutes.`
            });
        });
    }
