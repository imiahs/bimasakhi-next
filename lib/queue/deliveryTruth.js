...
function deriveDeliveryStatus(message, logs) {
    ...
    const latestTerminalLog = [...logs].reverse().find((log) => {
        if (log.state === 'DELIVERED' || log.state === 'CANCELED' || log.state === 'FAILED') {
            return true;
        }

        return log.state === 'ERROR' && (!log.nextDeliveryTime || log.nextDeliveryTime <= Date.now());
    }) || null;
    ...
    if (latestTerminalLog) {
        if (latestTerminalLog.state === 'DELIVERED') return 'delivered';
        if (latestTerminalLog.state === 'CANCELED') return 'cancelled';
        return 'failed';
    }
...
export async function getDeliveryHealthMetrics({ recentHours = 24, stuckMinutes = 15 } = {}) {
    ...
    const [recentResult, stuckResult] = await Promise.all([
        supabase.from('external_delivery_logs')
            .select('status, operator_status, delivered_at, failed_at, last_provider_event_at, published_at')
    ...
    const recentFailures = recentRows.filter((row) =>
        ['failed', 'cancelled'].includes(row.status)
        && (!row.operator_status || row.operator_status === 'active')
    ).length;
    const recentDelivered = recentRows.filter((row) => row.status === 'delivered').length;
    const recentTerminal = recentDelivered + recentFailures;
